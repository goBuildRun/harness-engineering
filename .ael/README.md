# BuildRun Agent Engineering Lifecycle Runtime Internals

本目录是 BuildRun Agent Engineering Lifecycle 的运行时内部区，物理路径为 `buildrun-agent-engineering-lifecycle/.ael/`，保存可复用的 Agent 配置、规则、脚本、模板、工作流和 Work Item provider。
...
- buildrun-agent-engineering-lifecycle / product workspace 分层：[../docs/architecture/workspace.md](../docs/architecture/workspace.md)
...
| `ael-manifest.yaml` | AEL runtime 自身必备文件清单 |
...
统一入口：`.ael/scripts/ael plan|start|status|finish`。其中 `plan`、`start`、`finish` 是主动作，`status` 只读；`standard/strict` 先使用 `plan`，低风险 `lite` 可由 `start` 生成最小绑定。workspace 审计、活动任务迁移和 commit 判定作为内部管理能力存在，不扩展日常公共操作面。受控 Git 接收点或发布入口未验证前只能显示 `local` 或 `guarded`。

| 接口 | 用途 |
|------|------|
| `ael_init.sh` | 登记、切换和列出产品 |
| `pretty.sh` / `json_pretty.py` | 将 AEL JSON 输出格式化给人阅读 |
| `ael_knowledge.sh` | 初始化产品侧知识与证据目录 |
| `ael_intake.sh` | 扫描已有项目代码/文档并生成接入候选 |
...
| `ael_growth.sh` | 捕捉、扫描、review 并应用可审阅成长候选 |
...
- runtime 内部规则和模板属于 buildrun-agent-engineering-lifecycle。
- 产品规格、任务包、运行状态、QA 证据和产品知识属于产品仓库的 `ael-workspace/`。
