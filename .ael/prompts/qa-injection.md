...
2. `bash .ael/scripts/structure_guard.sh --diff` → pass
3. `bash .ael/scripts/plan_sync_check.sh` → pass
4. 测试覆盖边界、安全、scope
...
```bash
bash .ael/scripts/qa_sign_off.sh <TASK_ID> pass '<说明>'
bash .ael/scripts/subagent-pr-gate.sh <TASK_ID>
bash .ael/scripts/task_workspace.sh qa-path <TASK_ID>   # 排障：凭证路径
```
