...
def rel_to_harness(path: Path, ael_root: Path) -> str:
    try:
        return str(path.relative_to(ael_root)).replace("\\", "/")
    except ValueError:
        return path.name
...
def resolve_link_target(source: Path, product_root: Path, target: str) -> Path | None:
    ...
    # 从产品根再试（跨 buildrun-agent-engineering-lifecycle / product workspace 边界）
    alt = (product_root / clean.lstrip("./")).resolve()
    if alt.is_file() or alt.is_dir():
        return alt

    # ael-workspace/...、phase0/... 或 buildrun-agent-engineering-lifecycle/... 显式路径
    if clean.startswith("ael-workspace/") or clean.startswith("phase0/") or clean.startswith("buildrun-agent-engineering-lifecycle/"):
...
def include_reference_doc(path: Path, ael_root: Path, profile: str) -> bool:
    try:
        rel = path.relative_to(ael_root)
    except ValueError:
        return True
    parts = rel.parts
    if len(parts) >= 3 and parts[0] == "docs" and parts[1] == "references":
        ref_profile = parts[2]
        if ref_profile != "index.md" and path.is_relative_to(ael_root / "docs" / "references" / ref_profile):
            return not profile or ref_profile == profile
    return True


def collect_markdown_files(ael_root: Path, product_root: Path) -> list[Path]:
    files: list[Path] = []
    profile = active_profile(product_root)
    files.extend(p for p in ael_root.glob("docs/**/*.md") if include_reference_doc(p, ael_root, profile))
    for name in ("README.md", "ARCHITECTURE.md", "AGENTS.md", "CLAUDE.md"):
        p = ael_root / name
        if p.is_file():
            files.append(p)
    files.extend((ael_root / ".ael").glob("**/*.md"))
    cursor_rules = ael_root / ".cursor/rules"
    if cursor_rules.is_dir():
        files.extend(cursor_rules.glob("**/*.mdc"))
    files.extend(ael_root.glob("tasks/**/*.md"))

    for sub in ("ael-workspace", "architecture"):
    ...
    for extra in (
        ...
        product_root / ".cursor/rules/buildrun-agent-engineering-lifecycle.mdc",
    ):
        if extra.is_file():
            files.append(extra)
...
def check_links(ael_root: Path, product_root: Path, md_files: list[Path]) -> list[str]:
    issues: list[str] = []
    anchor_cache: dict[Path, set[str]] = {}
    for md in md_files:
        rel_s = rel_to_harness(md, ael_root)
...
def check_legacy_link_labels(ael_root: Path, md_files: list[Path]) -> list[str]:
...
def check_stale_patterns(ael_root: Path, md_files: list[Path]) -> list[str]:
    issues: list[str] = []
    for md in md_files:
        rel_s = rel_to_harness(md, ael_root)
        ...
        for i, line in enumerate(prose.splitlines(), start=1):
            ...
            if AEL_TASKS_OUTPUT.search(line) and not line_allowed(rel_s, line, "STALE_TASKS"):
                issues.append(f"STALE_TASKS: 任务产出应在 ael-workspace/planning/tasks/ @ {rel_s}:{i}")

            if BARE_AGENT_WS.search(line) and not line_allowed(rel_s, line, "STALE_WS"):
                issues.append(f"STALE_WS: 工作区应在 ael-workspace/runs/ @ {rel_s}:{i}")

    return issues


def check_legacy_tool_ref(ael_root: Path) -> list[str]:
    issues: list[str] = []
    for p in ael_root.rglob("*.md"):
        try:
            if "tools/check.sh" in p.read_text(encoding="utf-8"):
                issues.append(f"STALE_REF:{p.relative_to(ael_root)}")
        except OSError:
            pass
    return issues


def check_entropy_files(ael_root: Path, product_root: Path) -> list[str]:
    issues: list[str] = []
    for root in (ael_root,):
        if not root.exists():
            continue
...
def check_entry_doc_shape(ael_root: Path) -> list[str]:
    ...
    architecture = ael_root / "ARCHITECTURE.md"
...
def check_location_independent_docs(ael_root: Path, product_root: Path, md_files: list[Path]) -> list[str]:
    issues: list[str] = []
    for md in md_files:
        rel_s = rel_to_harness(md, ael_root)
        text = md.read_text(encoding="utf-8", errors="ignore")
        if md.is_relative_to(ael_root):
            if "../ael-workspace" in text:
                issues.append(f"LOCATION_COUPLED_DOC:{rel_s}: use $PRODUCT_ROOT/ael-workspace")
            if "cd ../../buildrun-agent-engineering-lifecycle" in text:
                issues.append(f"LOCATION_COUPLED_DOC:{rel_s}: buildrun-agent-engineering-lifecycle can be anywhere")
        elif md.is_relative_to(product_root / "ael-workspace"):
            if "runtime 当前位于" in text or "cd ../../buildrun-agent-engineering-lifecycle" in text:
                issues.append(f"PRODUCT_WORKSPACE_COUPLED:{rel_s}: do not pin buildrun-agent-engineering-lifecycle path")
    return issues


def main() -> int:
    if len(sys.argv) < 2:
        emit("block", "USAGE: doc_gardening_check.py <ael_root> [product_root]")
        return 0

    ael_root = Path(sys.argv[1]).resolve()
    product_root = Path(sys.argv[2]).resolve() if len(sys.argv) > 2 else ael_root.parent

    md_files = collect_markdown_files(ael_root, product_root)
    issues: list[str] = []
    issues.extend(check_links(ael_root, product_root, md_files))
    issues.extend(check_legacy_link_labels(ael_root, md_files))
    issues.extend(check_stale_patterns(ael_root, md_files))
    issues.extend(check_legacy_tool_ref(ael_root))
    issues.extend(check_entropy_files(ael_root, product_root))
    issues.extend(check_entry_doc_shape(ael_root))
    issues.extend(check_location_independent_docs(ael_root, product_root, md_files))
