#!/usr/bin/env python3
"""Compatibility path resolver for product AEL workspaces.
...
class Phase0Layout:
    ...
    def to_dict(self) -> dict[str, str]:
        return {
            ...
            "ael_name": self.ael_name,
            "product_id": self.product_id,
            "ael_profile": self.ael_profile,
...
def resolve_product_root(ael_root: Path, product_root: Path | None = None, product_id: str = "") -> Path:
    if product_root is not None:
        return product_root.resolve()
    return resolve_product_context(ael_root, product_id=product_id).root
...
def load_layout(ael_root: Path | None = None, product_root: Path | None = None, product_id: str = "") -> Phase0Layout:
    hr = (ael_root or ael_root_from_script()).resolve()
    ...
    return Phase0Layout(
        ...
        bmad_output_root=_join_under(mr, str(bmad.get("output_root") or "ael-workspace/bmad-output")),
        ael_name=str(harness.get("name") or DEFAULT_HARNESS["name"]),
        product_id=str(harness.get("product_id") or DEFAULT_HARNESS["product_id"]),
        ael_profile=str(harness.get("profile") or DEFAULT_HARNESS["profile"]),
...
def active_planning_gate_path(layout: Phase0Layout) -> Path:
    ...
    ci_gate = os.environ.get("AEL_CI_PLANNING_GATE", "").strip()
...
def main() -> int:
    ...
    p_ensure.add_argument("--ael-root", default="")
    p_ensure.add_argument("--product-root", default="")
    p_ensure.add_argument("--product-id", default="")

    args = parser.parse_args()
    hr = Path(args.ael_root).resolve() if args.ael_root else ael_root_from_script()
