...
from ael_cycle_stages import begin_stage, finish_stage, stage_start_readiness
...
from ael_gate_work_items import committed_task_binding, validate_planning_credential
from ael_task_resolution import valid_task_id
from ael_timing import budget_status, ledger_path, read_events, summarize_events
...
def validate_current_qa_evidence(
    ...
    try:
        ...
        active_path = task_id_path if os.environ.get("AEL_TASK_ID") else layout.runs_root / "active_task.json"
        ...
        command = [
            "python3", str(harness / ".ael/scripts/qa_fanout.py"),
            "--ael-root", str(harness), "--product-root", str(product),
            "--task-dir", task_dir,
        ]
        completed = subprocess.run(
            ...
            timeout=float(os.environ.get("AEL_QA_TIMEOUT_SECONDS", "120")) + 5,
        )
...
def validate_current_release_evidence(
    ...
    if strict.get("decision") == "pass":
        from ael_strict_gate import validate as validate_strict_evidence
