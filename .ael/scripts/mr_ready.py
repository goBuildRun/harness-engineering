#!/usr/bin/env python3
"""Product MR readiness gate for buildrun-agent-engineering-lifecycle."""
...
def main() -> int:
    ...
    parser.add_argument("--agent-review-receipt", default=os.environ.get("AEL_AGENT_REVIEW_RECEIPT", ""))
    parser.add_argument(
        "--require-agent-review", action="store_true",
        default=os.environ.get("AEL_AGENT_REVIEW_REQUIRED", "").lower() == "true",
    )
    ...
    if runs_changes:
        emit("block", "MR_RUNS_STATE_DIRTY: ael-workspace/runs 是本地状态，不应进入 MR", files=runs_changes)
        return 0
    if not changed and not args.allow_no_changes:
        emit("block", "MR_NO_CHANGES: 产品仓库没有待提交变更")
        return 0

    check = run_json(["bash", str(ael_root / ".ael/scripts/check.sh")], ael_root)
