#!/usr/bin/env bash
# agent_start.sh — AEL Execution 启动（Work Item 驱动，自动恢复 planning gate）
set -euo pipefail
...
source "$SCRIPT_DIR/ael_output.sh"
...
if ! python3 -c 'import sys; sys.path.insert(0, sys.argv[1]); from ael_task_resolution import valid_task_id; sys.exit(0 if valid_task_id(sys.argv[2]) else 1)' "$SCRIPT_DIR" "$WORK_ITEM_ID"; then
...
if [[ -z "${AEL_TASK_SCOPED:-}" ]]; then
  AEL_TASK_SCOPED=0
  if python3 - "$SCRIPT_DIR" "$AEL_ROOT" "$PRODUCT_ROOT" "$WORK_ITEM_ID" <<'PY'
import sys
...
export AEL_TASK_SCOPED
[[ "$AEL_TASK_SCOPED" == "1" ]] && export AEL_TASK_ID="$WORK_ITEM_ID"

ACTIVATE_ARGS=(activate "$WORK_ITEM_ID")
[[ "$AEL_TASK_SCOPED" == "1" ]] && ACTIVATE_ARGS+=(--task-scoped)
...
if [[ "$VALID" != "OK" ]]; then
  python3 "$EMIT" block "PLANNING_GATE_INVALID: 重新执行 planning_gate 或确认 ael-workspace/planning/tasks/ 含 planning_gate_pass.json（兼容 phase0_pass.json）"
  exit 0
fi
...
[[ "$GATE_SOURCE" == "harness-plan-batch" ]] && export AEL_LEAN_FLOW=1
if [[ -z "$BOUND_ID" ]]; then
...
LIFECYCLE_PREFLIGHT=$(python3 "$SCRIPT_DIR/ael_lifecycle_preflight.py" \
...
if [[ "$LEVEL" == "L3" ]] && ! echo "$LIFECYCLE_PREFLIGHT" | python3 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if d.get('decision')=='pass' else 1)" 2>/dev/null; then
  ael_print_json "$LIFECYCLE_PREFLIGHT"
  exit 0
fi
...
if [[ -f "$PLAN_FILE" ]]; then
  ...
  done < <(python3 - "$SCRIPT_DIR" "$AEL_ROOT" "$PRODUCT_ROOT" "$PLAN_FILE" <<'PY'
import sys
from pathlib import Path
...
import sys
sys.path.insert(0, sys.argv[1])
from ael_tier import classify_tier
raw = sys.argv[2:]
...
RUNTIME_START=$("$SCRIPT_DIR/ael" start "$WORK_ITEM_ID" \
...
if [[ "$GATE_SOURCE" == "harness-plan-batch" ]]; then
  ...
  if [[ "$CURRENT_STAGE" == "takeover" ]]; then
    "$SCRIPT_DIR/ael" stage "$WORK_ITEM_ID" end takeover --reason BATCH_PREFLIGHT_COMPLETE >/dev/null || true
    CURRENT_STAGE=""
  fi
  if [[ -z "$CURRENT_STAGE" ]]; then
    PLANNING_STAGE=$("$SCRIPT_DIR/ael" stage "$WORK_ITEM_ID" start planning --reason BATCH_PLANNING_COMPLETE || true)
    if echo "$PLANNING_STAGE" | python3 -c "import sys,json; sys.exit(0 if json.load(sys.stdin).get('decision')=='pass' else 1)" 2>/dev/null; then
      "$SCRIPT_DIR/ael" stage "$WORK_ITEM_ID" end planning --reason BATCH_PLANNING_COMPLETE >/dev/null || true
    fi
  fi
fi
...
if [[ "$LEVEL" == "L3" ]]; then
  ...
  if [[ -z "$CURRENT_STAGE" ]]; then
    IMPLEMENTATION_STAGE=$("$SCRIPT_DIR/ael" stage "$WORK_ITEM_ID" start implementation_test || true)
    if ! echo "$IMPLEMENTATION_STAGE" | python3 -c "import sys,json; sys.exit(0 if json.load(sys.stdin).get('decision')=='pass' else 1)" 2>/dev/null; then
      ael_print_json "$IMPLEMENTATION_STAGE"
      exit 0
    fi
...
python3 - "$CONTEXT_FILE" "$LEGACY_CONTEXT" "$GATE_FILE" "$WORK_ITEM_ID" "$LEVEL" "$TASK_DIR" "$PRODUCT_ROOT" "$SCRIPT_DIR" "$TASKS_ROOT" "$AEL_ROOT" <<'PY'
import json, subprocess, sys
from pathlib import Path

ctx_path, legacy_path, gate_path, wi_id, level, task_dir, product_root, script_dir, tasks_root, ael_root = sys.argv[1:11]
...
def excerpt(path: Path, label: str, rel_path: str, limit: int = 2200) -> str:
    if not path.is_file():
        return f"### {label}\n\n- 路径: `{rel_path}`\n- 状态: 文件不存在；先运行 `ael_knowledge.sh ensure` 或确认产品 workspace 配置。\n"
...
try:
    ...
    from ael_knowledge import ensure
    from ael_context_index import render_index, write_index
    from workspace_paths import load_layout

    layout = load_layout(Path(ael_root).resolve(), Path(product_root).resolve())
    ensure(layout)
    ...
    knowledge_block = "\n".join(
        [
            ...
            "",
            f"- 状态: 读取产品知识失败：{exc}",
            "- 处理: 先运行 `ael_knowledge.sh ensure` 并确认产品侧 `ael-workspace/project.yaml`。",
        ]
    )
...
- Work Item ID: {wi_id}
- 工作区: ael-workspace/runs/tasks/{wi_id}/
- 凭证: ael-workspace/planning/tasks/.../planning_gate_pass.json（兼容 phase0_pass.json，已激活）
...
## Agent 工作指南
1. buildrun-agent-engineering-lifecycle/AGENTS.md → docs/getting-started/cli.md → docs/execution/collaboration.md
2. Lead: tasks-dag.md + 03-实施方案路径表
3. 写前: structure_guard.sh --path；写后: structure_guard.sh --diff + plan_sync_check.sh
4. 完成: qa_sign_off → subagent-pr-gate → ael_growth review/apply → check.sh → provider ready/review → MR；done 仅由 merge/release CI receipt 写入
"""
Path(ctx_path).write_text(text, encoding="utf-8")
Path(legacy_path).write_text(text, encoding="utf-8")
PY

bash "$SCRIPT_DIR/work_item.sh" close --id "$WORK_ITEM_ID" --status in_progress --note "AEL Execution started" >/dev/null 2>&1 || true

echo "[*] 产品根: $PRODUCT_ROOT" >&2
echo "[*] Execution 工作区: ael-workspace/runs/tasks/${WORK_ITEM_ID}/" >&2
echo "[*] 分支建议: feature/wi-${WORK_ITEM_ID}" >&2
python3 "$EMIT" pass "AGENT_READY: AEL Execution 已启动（Work Item ${WORK_ITEM_ID}）→ ${CONTEXT_FILE}"
