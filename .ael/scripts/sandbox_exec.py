#!/usr/bin/env python3
"""Run a test command without shell eval and emit AEL JSON."""
...
def docker_command(argv: list[str], cwd: Path) -> list[str]:
    image = os.environ.get("AEL_SANDBOX_IMAGE") or "python:3.11-slim"
    network = os.environ.get("AEL_SANDBOX_DOCKER_NETWORK") or "none"
    return [
        "docker",
        "run",
        "--rm",
...
def run_remote(argv: list[str], timeout: int) -> tuple[bool, str]:
    endpoint = os.environ.get("AEL_SANDBOX_REMOTE_URL", "").strip()
    workspace_ref = os.environ.get("AEL_SANDBOX_WORKSPACE_REF", "").strip()
    token = os.environ.get("AEL_SANDBOX_REMOTE_TOKEN", "").strip()
    subject = os.environ.get("AEL_SANDBOX_SUBJECT_DIGEST", "").strip()
    parsed = urllib.parse.urlparse(endpoint)
    if parsed.scheme != "https" or not parsed.netloc:
        return False, "REMOTE_SANDBOX_CONFIG_INVALID: AEL_SANDBOX_REMOTE_URL must use https"
...
def parse_timeout(raw: str | None) -> tuple[int, str | None]:
    ...
    try:
        ...
        return 0, "SANDBOX_TIMEOUT_INVALID: AEL_SANDBOX_TIMEOUT_SECONDS must be a positive integer"
    if timeout <= 0:
        return 0, "SANDBOX_TIMEOUT_INVALID: AEL_SANDBOX_TIMEOUT_SECONDS must be a positive integer"
    return timeout, None
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=os.environ.get("AEL_SANDBOX_BACKEND") or "controlled",
    )
    ...
    timeout, timeout_error = parse_timeout(
        os.environ.get("AEL_SANDBOX_TIMEOUT_SECONDS"),
    )
    if timeout_error:
