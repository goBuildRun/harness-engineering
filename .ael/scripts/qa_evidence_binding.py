...
from ael_gate_execution import gate_input_digest
from ael_output import dump_json
from ael_runtime import canonical_digest, load_result, now, policy_for, subject_for
...
def host_reviewer_identity(work_item_id: str, task_id: str) -> dict[str, Any]:
    path = Path(os.environ.get("AEL_QA_REVIEWER_IDENTITY_RECEIPT", "").strip())
...
def _run_mechanical_gates(harness: Path, product: Path) -> dict[str, Any]:
    scripts = harness / ".ael/scripts"
    env = {**os.environ, "AEL_PRODUCT_ROOT": str(product), "AEL_GATE_READ_ONLY": "1"}
    for name, command in (
        ("structure", ["bash", str(scripts / "structure_guard.sh"), "--diff"]),
        ("plan_sync", ["bash", str(scripts / "plan_sync_check.sh")]),
    ):
...
def main() -> int:
    ...
    parser.add_argument("--product-root", default=os.environ.get("AEL_PRODUCT_ROOT", ""))
    ...
    try:
        ...
        common = (Path(args.ael_root).resolve(), Path(args.product_root).resolve(), args.work_item, paths)
