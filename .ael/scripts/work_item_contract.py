...
def product_root_for(path: Path) -> Path:
    for parent in (path.parent, *path.parents):
        if parent.name == "ael-workspace":
            return parent.parent
        if (parent / "ael-workspace/project.yaml").is_file():
            return parent
    return path.parent
...
def build_work_item_note(spec_path: Path, acceptance_title: str, assignee: str = "") -> str:
    ...
    exec_hint = "ael-workspace/planning/exec-plans/active/<待补>"
    task_hint = "ael-workspace/planning/tasks/<待 Planning Gate 后回填>"
...
def gate_check(ael_root: Path, level: str, task_dir: str | None) -> dict[str, Any]:
    cfg = load_config(ael_root)
    provider = get_provider(ael_root)
    failures: list[str] = []
    ...
    if not wi_id:
        ...
        if not spec:
            ...
            try:
                spec_path, spec = resolve_product_spec_path(ael_root, product_root, spec)
