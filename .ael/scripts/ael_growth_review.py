#!/usr/bin/env python3
"""Review and apply AEL growth candidates to product knowledge."""
...
from ael_knowledge import template_path
...
def marker_lines(kind: str, report_rel: str, *, legacy: bool = False) -> tuple[str, str]:
    ...
    begin = f"<!-- BEGIN buildrun-agent-engineering-lifecycle:growth:{kind}:{key} -->"
    end = f"<!-- END buildrun-agent-engineering-lifecycle:growth:{kind}:{key} -->"
    return begin, end
