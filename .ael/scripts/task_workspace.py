...
def cmd_infer_mr(layout: Phase0Layout) -> int:
    env_dir = os.environ.get("AEL_TASK_DIR", "").strip()
    if env_dir:
        p = Path(env_dir)
        if not p.is_absolute():
            p = (layout.tasks / env_dir).resolve() if not p.exists() else (layout.product_root / env_dir).resolve()
        if p.is_dir():
            emit("pass", "MR_TASK_DIR", task_dir=str(p.resolve()), source="AEL_TASK_DIR")
            return 0
        emit("block", f"AEL_TASK_DIR_INVALID: {env_dir}")
        return 0
    ...
    if inferred:
        ...
        return 0
    rel = layout.rel(layout.tasks)
    emit("block", f"MR_TASK_DIR_UNKNOWN: 设置 AEL_TASK_DIR 或 MR 仅变更一个 {rel}/<id>/ 目录")
    return 0
