#!/usr/bin/env python3
"""Tier-aware structured adapters for existing AEL gates."""
...
from process_control import run_process_group
from ael_strict_gate import validate as _strict_evidence
...
def checks_for_tier(checks: dict[str, dict[str, Any]], tier: str) -> dict[str, dict[str, Any]]:
    ...
    if os.environ.get("AEL_LEAN_FLOW") == "1":
...
def _commands(harness: Path) -> dict[str, list[str]]:
    ...
    return {
        "harness": ["bash", str(scripts / "validate_ael.sh")],
        "structure": ["bash", str(scripts / "structure_guard.sh"), "--diff"],
        "diff_integrity": [
            "python3", str(scripts / "diff_integrity_check.py"),
            "--ael-root", str(harness),
        ],
        "plan_sync": ["bash", str(scripts / "plan_sync_check.sh")],
        "dag_sync": ["bash", str(scripts / "dag_sync_check.sh")],
        "qa_evidence": [
            "python3", str(scripts / "qa_fanout.py"),
            "--ael-root", str(harness), "--product-root", "__PRODUCT_ROOT__",
            "--compatibility-source", str(scripts / "qa_evidence_check.sh"),
        ],
        "knowledge": ["bash", str(scripts / "ael_knowledge.sh"), "check-planning"],
        "growth_review": ["bash", str(scripts / "ael_growth.sh"), "review-status"],
        "growth_freshness": ["bash", str(scripts / "ael_growth.sh"), "freshness"],
        "growth_release": [
            "python3", str(scripts / "ael_growth_release.py"),
            "--ael-root", str(harness), "--product-root", "__PRODUCT_ROOT__",
        ],
...
def run_gate_plan(harness: Path, product: Path, *, tier: str,
    ...
    if (
        os.environ.get("AEL_LEAN_FLOW") == "1"
    ...
    if ci_task_id:
        env.update({
            "AEL_CI_TASK_ID": ci_task_id,
            "AEL_CI_PLANNING_GATE": str(planning_gate),
            "AEL_CI_CHANGED_FILES_DIGEST": canonical_digest(changed_files or []),
        })
    if read_only:
        env.update({"CI": "true", "AEL_GATE_READ_ONLY": "1"})
    ...
    for name in required_gates:
        if name in {"planning", "strict_evidence"}:
            continue
        if name == "browser_qa":
            url = os.environ.get("AEL_BROWSER_QA_URL", "")
            commands_to_run[name] = ([
                "python3", str(harness / ".ael/scripts/browser_qa.py"), url,
                "--action", "audit", "--ael-root", str(harness),
        ...
        if deadline is not None and strict_started >= deadline:
            ...
            input_digest = canonical_digest({
                ...
                "receipt_path_digest": canonical_digest(
                    os.environ.get("AEL_STRICT_EVIDENCE", ""),
                ),
            })
...
def main() -> int:
    ...
    result = run_gate_plan(
        Path(args.ael_root).resolve(), Path(args.product_root).resolve(),
