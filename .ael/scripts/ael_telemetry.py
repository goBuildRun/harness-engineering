...
def apply_usage_receipt(result: dict[str, Any], *, task_id: str,
                        ...
                        path: str = "") -> bool:
    raw = path or os.environ.get("AEL_USAGE_RECEIPT", "")
    if not raw:
        return False
    try:
...
def apply_automatic_usage(result: dict[str, Any], *, task_id: str,
    ...
    if os.environ.get("AEL_USAGE_RECEIPT", "").strip():
...
def enforce_budget(result: dict[str, Any]) -> None:
    limits = {
        "input_tokens": os.environ.get("AEL_BUDGET_INPUT_TOKENS", ""),
        "output_tokens": os.environ.get("AEL_BUDGET_OUTPUT_TOKENS", ""),
        "agent_calls": os.environ.get("AEL_BUDGET_AGENT_CALLS", ""),
    }
    exceeded = []
