#!/usr/bin/env python3
"""Canonical Git-object attestations for AEL-validated commits."""
...
def create_attestation(repo: Path, result: dict[str, Any], *, commit: str = "HEAD") -> dict[str, Any]:
    ...
    if result.get("decision") != "pass" or result.get("state") != "validated":
        return {"decision": "block", "reason": "AEL_RESULT_NOT_PASS"}
    task_id = str(result.get("task_id") or "")
    policy = str(result.get("policy_digest") or "")
    if not task_id or not policy:
        return {"decision": "block", "reason": "AEL_RESULT_BINDING_MISSING"}
    subject = result.get("subject") or {}
    if subject.get("kind") == "worktree":
        paths = subject.get("paths")
        if not isinstance(paths, list) or not paths or not all(isinstance(path, str) for path in paths):
            return {"decision": "block", "reason": "AEL_RESULT_SUBJECT_PATHS_MISSING"}
        if commit_subject(repo, sha, paths) != subject.get("digest"):
            return {"decision": "block", "reason": "AEL_RESULT_SUBJECT_MISMATCH"}
    elif subject.get("kind") == "commit":
        if subject.get("digest") != sha:
            return {"decision": "block", "reason": "AEL_RESULT_SUBJECT_MISMATCH"}
    else:
        return {"decision": "block", "reason": "AEL_RESULT_SUBJECT_INVALID"}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Internal Git-native AEL attestation helper")
    ...
    if args.action == "verify":
        ...
        try:
            ...
            outcome = {"decision": "block", "reason": "AEL_RESULT_INVALID"}
        else:
