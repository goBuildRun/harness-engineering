#!/usr/bin/env python3
"""Command handlers for the lean AEL runtime."""
...
from ael_candidate import bound_candidate, refresh_evidence, release_evidence_paths
from ael_output import dump_json
from ael_assurance import finalize
from ael_attestation import verify_attestation
from ael_cache import executed_check, reuse_check, tool_digest
from ael_gates import checks_for_tier, committed_work_item_resolution, run_gate_plan
from ael_telemetry import apply_automatic_usage, enforce_budget
from codex_usage_receipt import automatic_receipt
from ael_usage_ledger import aggregate_epic_usage, apply_story_usage, capture_usage_baseline
...
from ael_scope import execution_paths, paths_within_scope
from ael_state import invalidate_if_stale
from worktree_baseline import capture_baseline, changed_since_baseline
from ael_ci import cmd_ci_check
from ael_task_binding import resolve_start_work_item, strengthen_resumed_task
from ael_task_resolution import bind_active_task, valid_task_id
from ael_timing import budget_status, ledger_path, stage_budget_status
from ael_cycle_commands import (
...
from ael_lifecycle_preflight import discover as discover_lifecycle, preflight_resumed
import ael_finish
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if os.environ.get("AEL_TASK_SCOPED") == "1":
        shared_lock = path
    if shared_lock == path:
...
def _cmd_start_locked(
    ...
    result["task_scoped_state"] = os.environ.get("AEL_TASK_SCOPED") == "1"
    result["flow_policy"] = {
        "lean": os.environ.get("AEL_LEAN_FLOW") == "1",
        "source": "harness-plan-batch" if os.environ.get("AEL_LEAN_FLOW") == "1" else "legacy-start",
        "growth_release": os.environ.get("AEL_LEAN_FLOW") != "1",
        "persisted_at": now(),
    }
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    current_policy = policy_for(Path(args.ael_root).resolve(), product)
...
def cmd_finish(args: argparse.Namespace) -> int:
    return ael_finish.execute_finish(args, commands=sys.modules[__name__])
