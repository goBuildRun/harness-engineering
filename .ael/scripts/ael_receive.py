...
from ael_gates import run_gate_plan
from ael_gc_context import build_gc_context
from ael_gc_receipt import verify_receipt as verify_gc_receipt
from ael_output import dump_json
from ael_runtime import classify_tier, git_changed, mechanical_code_health, policy_for
...
def main() -> int:
    ...
    configured = tuple(args.protected_ref or os.environ.get(
        "AEL_PROTECTED_REFS", "refs/heads/main").split(","))
    repo = Path(args.repo).resolve()
    harness = Path(args.ael_root).resolve()
