#!/usr/bin/env python3
"""Run product-configured lint/test commands and emit AEL JSON."""
...
PY_SKIP_DIRS = {".git", ".venv", "venv", "__pycache__", "node_modules", "_bmad", "ael-workspace"}
...
def allowed_executables() -> set[str]:
    extra = {
        item.strip()
        for item in os.environ.get("AEL_QUALITY_ALLOWED_EXECUTABLES", "").split(",")
        if item.strip()
    }
...
def main() -> int:
    ...
    parser.add_argument("--timeout", type=int, default=int(os.environ.get("AEL_QUALITY_TIMEOUT_SECONDS") or "600"))
    ...
    strict = args.strict or required or os.environ.get("AEL_QUALITY_STRICT", "") in {"1", "true", "yes"}
