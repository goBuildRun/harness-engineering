...
from ael_provider_preflight import validate_receipt as validate_provider_preflight
...
from ael_provider_authority import validate_binding as validate_provider_authorities
...
def validate(
    ...
    path = Path(os.environ.get("AEL_STRICT_EVIDENCE", ""))
    try:
        receipt = json.loads(path.read_text(encoding="utf-8")) if str(path) != "." else {}
    except (OSError, json.JSONDecodeError):
        receipt = {}
    required = tuple(COMPONENT_AUTHORITIES)
