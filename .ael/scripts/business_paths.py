#!/usr/bin/env python3
"""Shared profile business path helpers for AEL gates."""
...
def _active_product_root(ael_root: Path) -> Path | None:
    try:
        return resolve_product_context(ael_root, include_legacy=False).root
    except ProductContextError:
        return None


def resolve_profile(ael_root: Path, product_root: Path | None = None, explicit_profile: str = "") -> str:
    if explicit_profile:
    ...
    if product_root:
        ...
        if profile:
            return profile
    active_root = _active_product_root(ael_root)
    if active_root:
        profile = _profile_from_product_root(active_root)
        if profile:
            return profile
    harness_cfg = _load_yaml_file(ael_root / ".ael" / "config.yaml")
    harness = harness_cfg.get("harness") or {}
    return str(harness.get("profile") or "generic")


def allowlist_path(ael_root: Path, profile: str = "", product_root: Path | None = None) -> Path:
    selected_profile = resolve_profile(ael_root, product_root, profile)
    profile_path = ael_root / ".ael" / "profiles" / selected_profile / "package-allowlist.yaml"
    if profile_path.is_file():
        return profile_path
    if selected_profile == "generic":
        return ael_root / ".ael" / "rules" / "package-allowlist.yaml"
    return profile_path
...
def load_business_roots(ael_root: Path, profile: str = "", product_root: Path | None = None) -> tuple[str, ...]:
    selected_product_root = product_root or _active_product_root(ael_root)
    product_config = _product_config_from_root(selected_product_root)
    platform_roots = _platform_business_roots(product_config)
    if platform_roots:
        return tuple(dict.fromkeys(platform_roots))

    rules_path = allowlist_path(ael_root, profile, product_root)
...
def main() -> int:
    ...
    product_root = Path(args.product_root).resolve() if args.product_root else _active_product_root(Path(args.ael_root).resolve())
    roots = load_business_roots(Path(args.ael_root).resolve(), args.profile, product_root)
