...
WORKSPACE_JSON=$(python3 "$SCRIPT_DIR/workspace_paths.py" --ael-root "$ROOT_DIR" --product-root "$PRODUCT_ROOT" json)
...
if ! python3 -c 'import sys; sys.path.insert(0, sys.argv[1]); from ael_task_resolution import valid_task_id; sys.exit(0 if valid_task_id(sys.argv[2]) else 1)' "$SCRIPT_DIR" "$TASK_ID"; then
...
if [[ ! -f "$QA_EVIDENCE_FILE" ]]; then
  python3 "$EMIT" block "VIOLATION_NO_QA: 缺少 ${QA_EVIDENCE_FILE}。请 qa-evaluator 运行: bash .ael/scripts/qa_sign_off.sh ${TASK_ID} pass '<说明>'"
  exit 0
fi
...
script_dir, ael_root, product_root, path, task_id = sys.argv[1:6]
...
if data.get("decision") != "pass":
    ...
    issues = validate_qa_json(
        ...
        load_layout(Path(ael_root), Path(product_root)),
    )
