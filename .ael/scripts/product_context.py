#!/usr/bin/env python3
"""Shared product context resolver for buildrun-agent-engineering-lifecycle scripts."""
...
class ProductContext:
    ...
    source: str
    product_id: str = ""
    workspace: str = "ael-workspace"
    config: str = "ael-workspace/project.yaml"
...
def _workspace_from_config(root: Path) -> str:
    ...
    return str(value or "ael-workspace").strip("/") or "ael-workspace"
...
def _context_from_root(
    ...
    product = dict(registered or find_registered_by_root(ael_root, root) or {})
    workspace = str(product.get("workspace") or _workspace_from_config(root) or "ael-workspace").strip("/")
...
def _context_from_registered(ael_root: Path, product: dict[str, Any], source: str, strict: bool) -> ProductContext | None:
    ...
    if root is None:
        ...
        return None
    return _context_from_root(ael_root, root, source, product_id=str(product.get("id") or ""), registered=product)
...
def _resolve_selector(
    ...
    if product_root:
        ...
        registered_by_root = find_registered_by_root(ael_root, root)
        if product_id:
            registered_by_id = find_registered_by_id(ael_root, product_id)
        ...
        return _context_from_root(ael_root, root, source, product_id=product_id, registered=registered_by_root)

    registered = find_registered_by_id(ael_root, product_id)
    if registered is None:
        if strict:
            raise ProductContextError(f"PRODUCT_NOT_REGISTERED: product_id={product_id}")
        return None
    return _context_from_registered(ael_root, registered, source, strict=strict)


def _env_selector(environ: Mapping[str, str]) -> tuple[str, str]:
    root = (
        environ.get("AEL_PRODUCT_ROOT")
        or environ.get(LEGACY_PRODUCT_ROOT_VAR)
        or ""
    )
    product_id = environ.get("AEL_PRODUCT_ID") or ""
    return root, product_id


def _cwd_context(ael_root: Path, cwd: Path) -> ProductContext | None:
    current = cwd.resolve()
    ...
    if registered:
        ...
        return _context_from_root(ael_root, root, "cwd:registry", product_id=str(product.get("id") or ""), registered=product)

    for candidate in (current, *current.parents):
        if any((candidate / rel).is_file() for rel in PRODUCT_CONFIG_CANDIDATES):
            return _context_from_root(ael_root, candidate.resolve(), "cwd:project-config")
    return None


def _active_context(ael_root: Path) -> ProductContext | None:
    path = active_path(ael_root)
    if not path.is_file():
        return None
    try:
...
def _marker_context(ael_root: Path) -> ProductContext | None:
    for marker in (".product-root",):
        path = ael_root / marker
        if not path.is_file():
            continue
        value = path.read_text(encoding="utf-8", errors="ignore").splitlines()[0].strip()
        if not value:
            continue
        target = Path(value).expanduser() if Path(value).is_absolute() else (ael_root / value)
        root = _safe_existing_dir(str(target))
        if root is not None:
            return _context_from_root(ael_root, root, f"marker:{marker}")
    return None


def _legacy_search_context(ael_root: Path) -> ProductContext | None:
    search = ael_root.resolve()
    for _ in range(4):
        if (
            ...
            return _context_from_root(ael_root, search, "legacy-search")
        if search.parent == search:
            break
        search = search.parent
    parent = ael_root.parent.resolve()
    return _context_from_root(ael_root, parent, "legacy-parent")
...
def resolve_product_context(
    ...
    ael_root = ael_root.resolve()

    explicit = _resolve_selector(ael_root, product_root=product_root, product_id=product_id, source="argument", strict=True)
    if explicit:
        return explicit
    ...
    if include_cwd:
        cwd_context = _cwd_context(ael_root, cwd or Path.cwd())
        if cwd_context:
            return cwd_context

    if include_active:
        active = _active_context(ael_root)
        if active:
            return active

    if include_legacy:
        marker = _marker_context(ael_root)
        if marker:
            return marker
        legacy = _legacy_search_context(ael_root)
        if legacy:
            return legacy
...
def resolve_product_root(ael_root: Path, **kwargs: Any) -> Path:
    return resolve_product_context(ael_root, **kwargs).root


def _ael_root_from_script() -> Path:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Resolve AEL product context")
    sub = parser.add_subparsers(dest="cmd", required=True)

    def add_common(p: argparse.ArgumentParser) -> None:
        p.add_argument("--ael-root", default="")
    ...
    ael_root = Path(args.ael_root).expanduser().resolve() if args.ael_root else _ael_root_from_script()
    try:
