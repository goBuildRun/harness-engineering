#!/usr/bin/env python3
"""Generate AEL self-growth reports from task evidence."""
...
from ael_growth_capture import capture_evidence
from ael_knowledge import ensure
from ael_growth_review import apply_review, resolve_report
...
def render(layout: Phase0Layout, candidates: list[tuple[Path, str]], work_item_id: str = "",
    ...
    lines = [
        ...
        *([f"- Work Item: `{work_item_id}`"] if work_item_id else []),
        ...
        "## 1. 候选沉淀项",
        "",
        "> 人工 review 后，运行 `ael_growth.sh apply-review` 把长期有效项写入 `CONTEXT.md` / `LESSONS.md`；架构文档和技术债 Work Item 由负责人单独确认。",
        "",
    ]
    if not candidates:
        lines.extend(["暂无候选。", ""])
