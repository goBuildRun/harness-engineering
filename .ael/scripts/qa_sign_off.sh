...
if [[ -n "$ACTIVE_WI" ]] && ! python3 -c 'import sys; sys.path.insert(0, sys.argv[1]); from ael_task_resolution import valid_task_id; sys.exit(0 if valid_task_id(sys.argv[2]) else 1)' "$SCRIPT_DIR" "$ACTIVE_WI"; then
...
if [[ "$DECISION" == "pass" ]]; then
  BUNDLE_STATUS=$(python3 "$SCRIPT_DIR/qa_evidence_binding.py" status \
    --ael-root "$AEL_ROOT" --product-root "$PRODUCT_ROOT" \
  ...
  if ! echo "$RECEIPT_BINDING" | python3 -c "import sys,json; d=json.load(sys.stdin); sys.exit(0 if d.get('decision')=='pass' else 1)" 2>/dev/null; then
    ael_print_json "$RECEIPT_BINDING"
    exit 0
  fi
