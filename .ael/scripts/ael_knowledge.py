#!/usr/bin/env python3
"""Project knowledge artifact bootstrap for BuildRun Agent Engineering Lifecycle."""
...
from ael_knowledge_parse import (
    artifact_kind,
    artifact_summary,
    compact,
    field_value,
...
PLANNING_BLOCK_BEGIN = "<!-- BEGIN buildrun-agent-engineering-lifecycle:planning-context -->"
PLANNING_BLOCK_END = "<!-- END buildrun-agent-engineering-lifecycle:planning-context -->"
...
def template_path(layout: Phase0Layout, name: str) -> Path:
    return layout.ael_root / ".ael" / "templates" / name
...
def build_planning_context_body(layout: Phase0Layout) -> tuple[str, dict[str, int]]:
    ...
    lines = [
        ...
        "- **维护方式**：本区块由 `ael_knowledge.sh sync-planning` 管理；需要补充语义解释时，可在本文件其他人工维护区补充。",
        "",
        "### 产品规格 / 产品蓝图",
        "",
    ]
