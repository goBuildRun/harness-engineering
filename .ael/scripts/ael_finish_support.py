...
def execute_finish(
    ...
    product, harness = commands.Path(args.product_root).resolve(), commands.Path(args.ael_root).resolve()
    ...
    previous_scope = os.environ.get("AEL_TASK_SCOPED")
    previous_task_id = os.environ.get("AEL_TASK_ID")
    previous_lean = os.environ.get("AEL_LEAN_FLOW")
    if task_scoped:
        os.environ["AEL_TASK_SCOPED"] = "1"
        os.environ["AEL_TASK_ID"] = task_id
    ...
    if isinstance(flow_policy, dict):
        if flow_policy["lean"]:
            os.environ["AEL_LEAN_FLOW"] = "1"
        else:
            os.environ.pop("AEL_LEAN_FLOW", None)
    ...
    try:
        ...
        if previous_scope is None:
            os.environ.pop("AEL_TASK_SCOPED", None)
        else:
            os.environ["AEL_TASK_SCOPED"] = previous_scope
        if previous_task_id is None:
            os.environ.pop("AEL_TASK_ID", None)
        else:
            os.environ["AEL_TASK_ID"] = previous_task_id
        if previous_lean is None:
            os.environ.pop("AEL_LEAN_FLOW", None)
        else:
            os.environ["AEL_LEAN_FLOW"] = previous_lean
