...
DEFAULT_AEL_NAME = "BuildRun Agent Engineering Lifecycle"
GENERIC_ID_PATTERN = r"^[A-Za-z0-9][A-Za-z0-9._:-]{1,127}$"


def load_dotenv(ael_root: Path) -> None:
    env_path = ael_root / ".env"
...
def active_product_root(ael_root: Path) -> Path | None:
    try:
        return resolve_product_context(ael_root, include_legacy=False).root
    except ProductContextError:
        explicit_vars = ("AEL_PRODUCT_ROOT", "AEL_PRODUCT_ID")
        if any(os.environ.get(var) for var in explicit_vars):
            raise
        return None


def load_product_work_item_config(ael_root: Path) -> dict[str, Any]:
    root = active_product_root(ael_root)
    if root is None:
        return {}
...
def load_config(ael_root: Path) -> dict[str, Any]:
    load_dotenv(ael_root)
    cfg: dict[str, Any] = _load_yaml(ael_root / ".ael/work-items/config.yaml")
    product_cfg = normalize_product_work_item_config(load_product_work_item_config(ael_root))
    if product_cfg:
...
def load_ael_name(ael_root: Path) -> str:
    product_root = active_product_root(ael_root)
    if product_root is not None:
        for rel in PRODUCT_CONFIG_CANDIDATES:
            ...
            if name:
                return name
    data = _load_yaml(ael_root / ".ael/config.yaml")
    name = str((data.get("harness") or {}).get("name") or DEFAULT_AEL_NAME).strip()
    return name or DEFAULT_AEL_NAME
