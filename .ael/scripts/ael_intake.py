#!/usr/bin/env python3
"""Brownfield product intake scan for the buildrun-agent-engineering-lifecycle."""
...
from ael_intake_review import apply_review, review_status, table_rows
from ael_intake_scan import collect
...
def render_report(layout: Phase0Layout, data: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "> 本报告是 brownfield 接入候选证据。它不会绕过人工判断直接修改长期知识；人工 review 后运行 `ael_intake.sh apply-review`，由 AEL 自动写入 `knowledge/CONTEXT.md` 与 `knowledge/LESSONS.md` 的受管区块。",
        "",
    ]
    if data["truncated"]:
    ...
    lines.extend(
        [
            "## 0. Review 工作台（先处理）",
            "",
            "> 把本节作为 intake 完成度看板。每一项处理完后改成 `[x]`；`ael_intake.sh review-status` 会以这里未勾选项作为阻断信号。",
            "",
            ...
            "- [ ] Review 第 5 节 CONTEXT 候选；确认后由 `apply-review` 写入 `ael-workspace/knowledge/CONTEXT.md`。",
            ...
            "- `证据位置 / Work Item` 写实际落点，例如 `ael-workspace/knowledge/CONTEXT.md#产品边界`、`docs/architecture.md#系统边界`、飞书任务 GUID，或 `忽略：上游参考代码，不追踪技术债`。",
    ...
    lines.extend(
        [
            ...
            "- 沉淀到 `ael-workspace/knowledge/REFERENCE_SYSTEMS.md`：核心能力、主要流程、可扩展点、禁改边界、升级风险。",
    ...
    lines.extend(
        [
            ...
            "2. 运行 `ael_intake.sh apply-review`，由 AEL 自动更新 `knowledge/CONTEXT.md`、`knowledge/LESSONS.md` 与 `knowledge/REFERENCE_SYSTEMS.md` 的受管区块。",
