...
- 运行或要求提供：`bash .ael/scripts/structure_guard.sh --diff` 的 `pass` 输出
...
```bash
# 通过
bash .ael/scripts/qa_sign_off.sh T1 pass '边界与安全测试已通过'

# 驳回
bash .ael/scripts/qa_sign_off.sh T1 fail '缺少越权用例'

# 业务 Agent 完结前
bash .ael/scripts/subagent-pr-gate.sh T1
```

主凭证：`runs/tasks/<work-item-id>/qa_approved_<TASK_ID>.json`；`runs/qa_approved_<TASK_ID>.json` 是当前兼容副本。模板见 `.ael/templates/qa-evidence.json`。
