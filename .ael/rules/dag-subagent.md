...
## 1. 拆解 DAG (有向无环图)
当前 L2/L3 兼容任务在 `ael-workspace/planning/tasks/<task>/tasks-dag.md` 维护 DAG；目标统一入口按任务复杂度生成或物化最小待办。DAG 的实现节点必须与 `03-实施方案.md` ID 对齐，不能放在产品仓库根目录。
...
示例：`bash .ael/scripts/feedback_planner.sh '[ ] T1 src/module/example.py 写失败测试 [ ] 实现 [ ] 沙箱验证'`

## 3. 风险匹配验证

- 所有实现任务都必须有可执行验证，且变更路径与任务契约一致。
- 要求独立 QA 的任务必须由 `qa-evaluator` 签字，并在成功调用 `.ael/scripts/subagent-pr-gate.sh T1` 后才能标记为 `[x]`。
- 未触发独立 QA 的任务由最终 AEL 结果证明合规，不生成虚假签章。
...
- 任何构建与测试行为，必须被包装在沙箱管道内执行：`bash .ael/scripts/run_in_sandbox.sh "你的原生命令"`。违者将被安全系统剥夺执行权限。
