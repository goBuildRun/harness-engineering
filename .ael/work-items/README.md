...
BuildRun Agent Engineering Lifecycle **不绑定**某一个协同系统。BMAD Planning 与 Lifecycle Execution 通过 **Work Item 抽象层** 对接外部任务系统。

Work Item 的职责是协同，不是保存 BMAD 产物本体。产品规格、执行计划和任务包的真相源始终是产品仓库的 `ael-workspace/planning/`；Teambition、飞书、Jira 只保存负责人、状态、讨论、摘要和链接。完整契约见 [../../docs/planning/work-item-contract.md](../../docs/planning/work-item-contract.md)。

> 目标状态由 `ael start/finish` 绑定并同步 Work Item：本地 `finish` 最多进入 ready/review，只有受控 Git 接收点或发布入口接受目标 commit 后才能写 `done`。下面 `work_item.sh` 是当前 adapter 与兼容命令参考，不是目标公开操作面。

同一个 buildrun-agent-engineering-lifecycle 可以同时管理多个产品：

| 产品 | 产品侧配置 | Work Item provider |
|------|------------|--------------------|
| A 项目 | `A/ael-workspace/project.yaml` | `teambition` |
| B 项目 | `B/ael-workspace/project.yaml` | `feishu` |
| C 项目 | `C/ael-workspace/project.yaml` | `jira` |
...
AEL 默认配置在 `.ael/work-items/config.yaml`，只提供 provider 默认值、默认 API host 和兼容策略。

产品侧 `ael-workspace/project.yaml` 可以保存非密钥配置，例如：
...
- `draft-spec` 从 `ael-workspace/planning/product-specs/<功能>.md` 的未绑定验收勾选项生成待确认任务草稿，不调用外部 API，也不修改 Product Spec。
- 人在 Codex 对话中确认任务标题、范围和负责人。
- `sync-spec` 创建 Work Item，在描述中写入目标、范围、负责人、AEL Links、Gate 状态和验收摘要。
...
```bash
bash .ael/scripts/work_item.sh diagnose
bash .ael/scripts/work_item.sh diagnose --id <work-item-id>
bash .ael/scripts/work_item.sh capabilities
bash .ael/scripts/work_item.sh scenario-configs --keyword 任务
...
bash .ael/scripts/work_item.sh verify --id <work-item-id> --level L2
bash .ael/scripts/work_item.sh pull --id <work-item-id>
bash .ael/scripts/work_item.sh list-mine
# 普通调用只允许推进到待发布；本地 done 会固定阻断
bash .ael/scripts/work_item.sh close --id <work-item-id> --status ready_to_release
```
...
# 2. 或提供 Open App 凭证，AEL 会按官方 JWT 规则本地签发 appAccessToken
...
- `list-mine`：优先拉取项目任务并按执行者 `executorId` 过滤；若 Teambition 列表接口漏返当前产品已同步的 Work Item，则从 `ael-workspace/planning/product-specs/` 的未勾选 `#taskId` 逐个 `pull` 兜底，并过滤 `isDone=true`
...
AEL 状态到 Teambition 阶段的默认映射：

| AEL status | `stage_id_map` key | Teambition 阶段 |
|----------------|--------------------|-----------------|
| `pending` / `todo` | `pending` | 待处理 |
| `bmad_planning` / `planning_gate_ready` | `design` | 设计中 |
| `in_progress` / `ael_execution_started` | `in_progress` | 开发中 |
...
- `diagnose --create-smoke-title "AEL smoke"`：显式创建一个 smoke 任务，用于真实写入联调；默认 diagnose 不写飞书
...
# 2. 真实凭证只测 token
WORK_ITEM_PROVIDER=feishu bash .ael/scripts/work_item.sh diagnose

# 2.1 如清单 diagnose 返回 1470403，先由清单 owner/editor 的 user token 把 app 加为清单 editor
FEISHU_USER_ACCESS_TOKEN=<user_access_token> \
WORK_ITEM_PROVIDER=feishu bash .ael/scripts/work_item.sh feishu-tasklist-member

# 3. 只读校验一个已有飞书任务
WORK_ITEM_PROVIDER=feishu bash .ael/scripts/work_item.sh diagnose --id <task_guid>

# 4. 显式写入 smoke 任务（确认要在飞书里创建任务时才执行）
WORK_ITEM_PROVIDER=feishu bash .ael/scripts/work_item.sh diagnose --create-smoke-title "AEL smoke"
```
