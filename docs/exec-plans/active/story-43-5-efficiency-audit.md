...
8. Historical audit-time shared CI credentials were replaced by per-task `runs/ci/<task-id>/` credentials in the current Harness runtime; product CI rollout and multi-Story concurrency remain deployment checks, not proven by this offline audit.
