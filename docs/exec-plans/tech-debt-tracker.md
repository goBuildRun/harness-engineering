...
| TD-019 | CI 对目标 commit 使用共享判定器生成 required check；本地 `finish` 只进入 ready/review，合并或发布后才能关闭 Work Item | 高 | partially done：GitHub PR/merge commit 双阶段 check、release/provider root workflows 与 terminal receipt gate 已落地；2026-08-13 live audit 确认 workflow 可见且 Actions 已启用，但尚无真实 run，`main` 也未设置 required rule |
