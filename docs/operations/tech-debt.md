...
当前执行顺序和验收标准以 [Git-native AEL Upgrade](../plans/active/git-native-ael-upgrade.md) 为准；本表只保留债务状态，不作为第二套 backlog。
...
| TD-013 | Flow-X 式知识沉淀与成长候选报告 | — | **已偿还** 2026-06-20 · `ael_knowledge.py` + `ael_growth.py` |
...
| TD-016 | 产品侧 `quality.commands` 接入 CI/check | — | **已偿还** 2026-06-21 · `quality_commands.py` + AEL allowlist/危险 env 拦截 |
| TD-017 | 开源发布前防泄漏预检 | — | **已偿还** 2026-06-21 · `release_preflight.sh` |
| TD-018 | 实现统一 `plan/start/status/finish` 门面、原子 `result.json` 与实现/AEL 分项成本遥测；内部复用现有 gate，不新增并行流程 | 中 | partially done：统一入口、结构化 runner、绑定 usage receipt、GC 模型身份和 rollout 汇总已落地；低风险 `lite` 可由 `start` 生成最小绑定；仍需第三方 provider 真实 usage response 与至少 5 个 lite 样本 |
...
| TD-023 | 拆分超过 1000 行的 brownfield intake，隔离扫描策略、信号采集、报告渲染与知识沉淀 | — | **已偿还** 2026-08-11 · `ael_intake_{policy,scan,review}.py` 按职责拆分，入口和各模块均不超过 400 行并保留 CLI/import 契约 |
...
| TD-025 | 清理其余 runtime 超大 Python 模块并机械防止重新增长 | — | **已偿还** 2026-08-11 · growth review、knowledge parse、doc policy、workspace config、product registry、BMAD init 已按职责拆分；全局测试强制 `.ael/scripts/*.py` 单文件不超过 400 行 |
| TD-026 | Scope-change / hotfix 使用显式风险升级且不能绕过共同不变式 | — | **已偿还** 2026-08-11 · 复用 `ael start --kind ... --reason ...`；scope-change 最低 standard、hotfix 强制 strict，kind floor 有 CLI 与分类器测试 |
...
| ael-manifest | 单一 validate 清单 |
| CI 硬门禁 | structure + MR gates |
| Work Item 抽象层 + 多 provider 基础能力 | `work_item.sh` + Teambition 实测 + 飞书/Jira 基础 adapter |
| 文档对齐审计 | docs/ 与 AEL 实态、跨文档一致性（2026-06-18） |
| Cursor rules | `.cursor/rules/buildrun-agent-engineering-lifecycle.mdc` |
...
| Docker 沙箱后端 | `run_in_sandbox.sh` 通过 `AEL_SANDBOX_BACKEND=docker` 切换 |
| Browser QA setup/CI | `browser_qa_setup.sh` + 可选 `AEL_BROWSER_QA_URL` CI job |
