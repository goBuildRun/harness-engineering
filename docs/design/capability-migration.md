...
> 状态：Git-native implementation，2026-08-13。此表解释能力承载关系；`.ael/scripts/capability_contract.py` 与 `validate_ael.sh` 机械检查承载入口和 tier gate 未被静默删除。guarded/enforced 是否对某个产品生效仍以该产品实际 authority audit 为准。

| 来源能力 | 统一入口承载 | 当前状态 | 验收证据 |
|----------|--------------|----------|----------|
| OpenAI AEL：短导航、机械反馈、doc gardening | `start` 最小绑定；`finish` 包装现有 gate | 已承载 | `validate_ael`、`doc-gardening`、runtime tests |
...
| GC sweeper | `finish` 机械扫描并按 tier/信号要求一次独立结果；接受端重建 context 并验证 SSH receipt | 已接入，receipt 绑定 commit/task/policy/context/triggers/model/telemetry，缺失或跨 subject 复用 fail closed | `test_ael_runtime.py`、`test_ael_gc_context.py`、`test_ael_gc_receipt.py` |
...
| Git commit 接受 | canonical Git object + attestation/result refs + receive authority | local/guarded/enforced 框架链已实现并通过真实 bare Git push；产品实例须独立 audit | `ael_attestation.py`、`ael_receive.py`、`ael_enforced.py` |
