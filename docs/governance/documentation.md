...
3. [ARCHITECTURE.md](../../ARCHITECTURE.md)、[architecture/team-model.md](../architecture/team-model.md) 和 [architecture/workspace.md](../architecture/workspace.md) 分别维护系统架构、通用模型和 workspace 分层。
4. [getting-started/cli.md](../getting-started/cli.md) 维护三项主动作（`plan/start/finish`）和只读 `status`，并明确标注当前可执行的过渡期命令。
5. [design/lean-enforcement.md](../design/lean-enforcement.md) 维护精简强制执行目标。
6. [architecture/workflow.md](../architecture/workflow.md) 只解释流程结构。
7. [operations/maturity.md](../operations/maturity.md) 维护当前评分、缺口和优化优先级。
8. [architecture/overview.md](../architecture/overview.md) 只保留设计背景与历史全景。

同一命令、provider 配置、迁移规则或成熟度结论不得在上述文档中并行维护全文。

## 1.1 当前实现基准

以下值直接对应 runtime；变更代码时必须同步更新本节和 [getting-started/cli.md](../getting-started/cli.md)，否则文档审查失败：
...
| 参考能力路由 | ✅ 已对齐 | `docs/references.md` 将来源、保留能力和内部权威说明一一映射，避免静默删除或重复维护外部流程 |
...
| [references.md](../references.md) | 外部参考能力的路由与内部权威说明 |
| [planning/bmad-planning.md](../planning/bmad-planning.md) | BMAD Planning / BMAD Method 专章 |
| [operations/maturity.md](../operations/maturity.md) | 当前成熟度与优化优先级 |
| [design/lean-enforcement.md](../design/lean-enforcement.md) | 精简强制执行目标与演进约束 |
