...
> 精简强制执行的权威设计见 [lean-enforcement.md](./lean-enforcement.md)。

0. **AI 时代研发新体系** — 「已有项目接入 — 产品设计（**BMAD Method**）— 协作 — Harness（AI Coding）」联动；**BMAD Planning 必须经 BMAD Method 完成**，否则产品环路不闭合。
1. **双闭环加事实前导** — Brownfield Intake 建立已有项目事实，BMAD Planning **BMAD Method**（`bmad_method_gate.py`）回答「做对的事」，Harness Execution OpenAI 主执行回答「把事做对」。
...
6. **熵要持续偿还** — 按风险触发 GC；未当场处理的真实技术债记入 `docs/exec-plans/tech-debt-tracker.md`。
...
10. **新增必须替代** — 新入口、新产物或新规则必须说明替代对象；不能减少复杂度或覆盖新风险的机制不进入 Harness core。
