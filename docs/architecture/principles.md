...
0. **AI 时代研发新体系** — 「已有项目接入 — 产品设计（**BMAD Method**）— 协作 — AEL（AI Coding）」联动；**BMAD Planning 必须经 BMAD Method 完成**，否则产品环路不闭合。
...
3. **地图优于手册** — `AGENTS.md` 保持简短；细节沉入 `docs/` 与 `.ael/rules/`。
...
9. **结果强制，过程最小** — 接入产品没有有效 AEL 合规结果就不能完成、合并或发布；执行深度按风险分层，禁止用重复命令、重复证据和重复上下文代替治理。
