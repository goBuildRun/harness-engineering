...
本报告只使用 Codex rollout 的事件类型、时间戳、token counter、工具名与结果码，以及 AEL result/check receipt、QA/TEST/REVIEW 路径元数据、Growth 时间锚点、部署/Provider receipt、Git commit 时间和生命周期 readback 结果。不复制消息正文、密钥、生产请求参数或业务数据。
...
- AEL 自身 input/output Token 保持 `unknown`，AEL telemetry 保持 `partial`，L3 保持 `live_validation_pending`；这些状态不得由 Codex 或 Provider Token 反推。
...
| guarded commit + lifecycle mapping/readback | 10:41:29-10:52:48.596 | 0:11:19.596 | 0:01:34.451 | 0:09:45.145 | unknown | 1 compaction | post-ledger 4,482,222/24,700 | product commit, AEL mapping fix, task_complete |
...
- AEL `gate_duration_ms=370,897` is cumulative across attempts. Final `checks[].duration_ms` measured only `10,095ms`; the remaining `360,802ms` cannot be unambiguously assigned.
...
|---|---|---:|---:|---|
| 1 | implementation, TDD, repeated hardening and GC/context repair | 7:22:29; 15 DeerFlow commits; ~199.52M input | 76.98% | must retain but optimize + AEL defect rework |
| 2 | deploy preflight and late Provider contract repair | 45:11; actual Provider only 54.815s | 7.86% | must retain but optimize + AEL defect rework |
...
These mutually exclusive rows total `9:34:36.120`; rounded shares total `99.97%`. AEL gate time `6:10.897` is a nested, non-additive diagnostic equal to about `1.08%` of root wall. It must not be added to the Pareto shares; six reruns and zero cache hits amplified surrounding rework.

The dominant cause is Agent iteration/context growth/evidence orchestration, not Provider or gate execution. AEL cannot by itself prove why the long-lived Codex host repeatedly carried up to ~239k context; context index and stage ledger make the next run measurable but do not retroactively prove host behavior.
...
| `close` triggering `sync_planning` | AEL defect rework | removed from provider state mutation |
| full-subject invalidation of every gate | AEL defect rework | gate-specific dependency digest |
| six same-input finish loops | AEL defect rework | same-input ceiling 2, structured stop |
| production-only required/allowed call discovery | AEL defect rework | versioned offline call-contract preflight |
...
The iCloud-backed AEL worktree was also sampled with three read-only `git status --untracked-files=no` runs: `0.01s`, `0.05s`, and `0.00s`. The previously observed 30–90s Git waits were not reproduced, so their historical contribution remains `unknown` and is not assigned to Story 43.5.
...
| unknown telemetry | Agent active, AEL input/output Token, Judge and production P95 remain `unknown`; AEL telemetry remains `partial` and L3 remains `live_validation_pending`; no zero fill |
...
- The two counts above are historical candidate evidence. The current AEL HEAD `fd7b66e2b737b1944de35af47e571d6f35c5bcc1` passed the complete suite `622/622` on 2026-08-23; this does not change the historical Story 43.5 timeline.
- AEL validation: `AEL_VALID`; Python compile, Bash syntax, module-boundary and `git diff --check` checks passed.
...
3. A product-specific Provider adapter must implement the versioned offline/real contract and emit `harness-provider-evidence-v1`; AEL deliberately does not encode small-Zhangluo route names.
...
5. Context indexing and phase handoff bound AEL injection, but the Codex host must honor phase-session admission; uninstrumented host reasoning cannot be preempted by this repository.
