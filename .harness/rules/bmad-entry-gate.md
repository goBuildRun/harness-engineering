...
> BMAD Planning 产出目录由产品侧 **`harness-workspace/project.yaml`** 配置（默认 `harness-workspace/planning/`），**不在** `harness-engineering/` 内。解析：`.harness/scripts/workspace_paths.py`。
...
模板：`.harness/templates/product-spec.md`、`.harness/templates/exec-plan.md`

必须存在以下之一（路径相对 **`harness-workspace/planning/` 根**，见产品 `harness-workspace/project.yaml`）：
...
路径：`tasks/YYYY-MM-DD-<work-item-id>-<简称>/`（完整路径为 `harness-workspace/planning/tasks/...`）
...
任务卡模板：`harness-engineering/tasks/_templates/`（仅模板留在 Harness）。
...
TASK_DIR="$PRODUCT_ROOT/harness-workspace/planning/tasks/YYYY-MM-DD-<id>-简称"
python3 .harness/scripts/bmad_method_gate.py --level L2 --harness-root . \
  --task-dir "$TASK_DIR"
...
- Gate 1：已确认
- workspace 配置：产品 `harness-workspace/project.yaml` → `workspace.planning`
