...
> 路径权威：当前产品 `harness-workspace/project.yaml` 中的 active profile + `.harness/profiles/<profile>/package-allowlist.yaml`；产品领域说明按需读取 `docs/references/<profile>/index.md`。
> 机械校验：`bash .harness/scripts/structure_guard.sh --path <路径>`
...
```bash
bash .harness/scripts/structure_guard.sh --diff
```
