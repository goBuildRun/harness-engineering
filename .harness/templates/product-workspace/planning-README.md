...
本目录存放 **BMAD Planning** 的可交付物。产品侧目录使用 `planning/`，与 harness-engineering runtime（脚本、门禁、CI、工作流文档）分离。
...
| `bmad.output_root` | `harness-workspace/bmad-output` | BMAD 原生输出暂存区，含 planning/design/implementation/test artifacts |
| `bmad.normalized_planning_root` | `harness-workspace/planning` | 映射后的正式 BMAD Planning 产物目录 |

解析脚本：harness-engineering 的 `.harness/scripts/workspace_paths.py`。
初始化脚本：在 harness-engineering 目录执行 `bash .harness/scripts/harness_init.sh init ...`。
