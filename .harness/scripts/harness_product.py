...
def resolve_context(args: argparse.Namespace) -> ProductContext:
    harness_root = Path(args.harness_root).expanduser().resolve() if args.harness_root else harness_root_from_script()
...
def cmd_env(args: argparse.Namespace) -> int:
    try:
        ...
        return 2
    print(f"export HARNESS_PRODUCT_ROOT={shlex.quote(str(context.root))}")
    if context.product_id:
        print(f"export HARNESS_PRODUCT_ID={shlex.quote(context.product_id)}")
    else:
        print("unset HARNESS_PRODUCT_ID")
    return 0
...
def cmd_exec(args: argparse.Namespace) -> int:
    ...
    if not command:
        ...
        return 2
    env = dict(os.environ)
    env["HARNESS_PRODUCT_ROOT"] = str(context.root)
    if context.product_id:
        env["HARNESS_PRODUCT_ID"] = context.product_id
    else:
        env.pop("HARNESS_PRODUCT_ID", None)
    proc = subprocess.run(command, env=env, check=False)
    return proc.returncode


def main() -> int:
    parser = argparse.ArgumentParser(description="Resolve or pin a harness product context without mutating active-product.json")
