#!/usr/bin/env python3
"""Dependency-scoped input fingerprints for Harness gates."""
...
from harness_gate_tools import SHELL_TOOL_REF, tool_dependency_entries
...
def _manifest_inputs(harness: Path, candidates: CandidateManifest) -> str:
    manifest_path = harness / ".harness/harness-manifest.yaml"
    try:
        import yaml
    except ImportError:
    ...
    return canonical_digest({
        ...
        "agents": candidates.tree_digest(
            harness / ".harness/agents", allowed_root=harness, suffixes={".yaml", ".md"},
        ),
    })
    ...
    if name == "harness":
        value: Any = {
            **common,
            ...
            "config": candidates.digest(harness / ".harness/config.yaml", root=harness),
        }
    elif name == "structure":
        value = {
            **common, "paths": changed_paths, "project": candidates.digest(project, root=product),
            "rules": candidates.tree_digest(
                harness / ".harness/rules", allowed_root=harness, suffixes={".md", ".yaml"},
            ),
        }
        ...
        value = {
            **common,
            ...
            "context": candidates.digest(
                product / "harness-workspace/knowledge/CONTEXT.md", root=product,
            ),
            "project": candidates.digest(project, root=product),
        }
