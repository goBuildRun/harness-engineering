...
def _validate_closure_order(
    policy: dict[str, Any], *, work_item_id: str, completed_work_items: tuple[str, ...],
) -> tuple[int, list[str]]:
    ...
    if list(completed_work_items) != predecessors:
        raise ValueError("ACCEPTANCE_CLOSURE_ORDER_BLOCKED")
...
def validate_receipt(receipt: Any, *, work_item_id: str, expected_ref: str,
    ...
    try:
        ...
        closure_index, predecessors = _validate_closure_order(
            acceptance_policy,
            work_item_id=work_item_id,
            completed_work_items=trust_policy.completed_work_items,
        )
    except ValueError as exc:
        return False, str(exc)
