#!/usr/bin/env bash
# validate_harness.sh — 读取 harness-manifest.yaml 机械校验
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "$SCRIPT_DIR/../.." && pwd)"
EMIT="$SCRIPT_DIR/emit_json.py"
MANIFEST="$ROOT_DIR/.harness/harness-manifest.yaml"
...
for agent in m.get("required_agents") or []:
    p = root / f".harness/agents/{agent}.yaml"
    if not p.is_file():
...
if [[ "$RESULT" == OK ]]; then
  ...
  BUSINESS_SMOKE_PATH=$(python3 "$SCRIPT_DIR/business_paths.py" roots --harness-root "$ROOT_DIR" --product-root "$PRODUCT_ROOT" 2>/dev/null | python3 -c "import sys; roots=[line.strip().rstrip('/') for line in sys.stdin if line.strip()]; root=roots[0] if roots else 'harness-workspace'; print(root + '/smoke.py')" 2>/dev/null || echo "harness-workspace/smoke.py")
  ...
  json_pass "$FP" || SMOKE_FAILS+=("SMOKE_FEEDBACK_PLANNER_BUSINESS_PATH:$FP")
	  ...
	  STRUCT_GENERIC_README=$(python3 "$SCRIPT_DIR/structure_check.py" --harness-root "$ROOT_DIR" --product-root "$PRODUCT_ROOT" --profile generic --path README.md 2>/dev/null || true)
	  json_pass "$STRUCT_GENERIC_README" || SMOKE_FAILS+=("SMOKE_STRUCTURE_GENERIC_README:$STRUCT_GENERIC_README")

	  STRUCT_GENERIC_ENV=$(python3 "$SCRIPT_DIR/structure_check.py" --harness-root "$ROOT_DIR" --product-root "$PRODUCT_ROOT" --profile generic --path .env 2>/dev/null || true)
...
business_root = (load_business_roots(root, product_root=product_root) or ("harness-workspace/",))[0].rstrip("/")
...
business_root = (load_business_roots(root, product_root=_product_root) or ("harness-workspace/",))[0].rstrip("/")
...
PY
  ...
  json_pass "$SANDBOX_SMOKE" || SMOKE_FAILS+=("SMOKE_SANDBOX_TRUE:$SANDBOX_SMOKE")
    ...
    python3 "$EMIT" block "HARNESS_INVALID: $REASON"
    exit 0
  fi

  python3 "$EMIT" pass "HARNESS_VALID: manifest + smoke 全项通过"
elif [[ "$RESULT" == BLOCK:MISSING_PYYAML ]]; then
  python3 "$EMIT" block "HARNESS_INVALID: 请 pip install pyyaml"
else
  REASON="${RESULT#BLOCK:}"
  python3 "$EMIT" block "HARNESS_INVALID: $REASON"
fi
