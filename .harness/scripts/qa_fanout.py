...
def fanout(*, harness_root: Path, product_root: Path, task_dir_arg: str = "",
           timeout_seconds: float = 120.0) -> dict:
    layout = load_layout(harness_root.resolve(), product_root.resolve())
    ...
    contract_issues = table_issues + validate(
        rows, plan_text, load_business_roots(layout.harness_root, product_root=layout.product_root),
    )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=float(os.environ.get("HARNESS_QA_TIMEOUT_SECONDS", "120")),
    )
    args = parser.parse_args()
    try:
        result = fanout(
            harness_root=Path(args.harness_root), product_root=Path(args.product_root).resolve()
