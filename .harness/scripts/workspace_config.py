...
DEFAULT_PHASE0 = {
    "root": "harness-workspace/planning",
...
def _normalize_project_config(data: dict[str, Any]) -> dict[str, Any]:
    ...
    workspace_root = str(workspace.get("root") or "harness-workspace").strip("/")
    ...
    return {
        ...
        "harness": {
            "name": harness.get("name") or DEFAULT_HARNESS["name"],
            "profile": product.get("profile") or harness.get("profile") or DEFAULT_HARNESS["profile"],
            "product_name": product.get("name") or harness.get("product_name") or DEFAULT_HARNESS["product_name"],
            "product_id": product.get("id") or harness.get("product_id") or DEFAULT_HARNESS["product_id"],
            "task_contract": harness.get("task_contract") or DEFAULT_HARNESS["task_contract"],
...
def load_config(harness_root: Path, product_root: Path | None = None) -> dict[str, Any]:
    """Load harness-engineering defaults, then overlay product workspace config when present."""
    harness_config_path = harness_root / ".harness/config.yaml"
