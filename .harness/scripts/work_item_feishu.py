...
from acceptance_authority import TerminalAuthorization, terminal_authorization_matches
...
class FeishuProvider(FeishuBindingMixin, FeishuPayloadMixin, WorkItemProvider):
    ...
    def update_status(
        ...
        *,
        ...
        protected = requires_terminal_authority(status)
        if protected and not terminal_authorization_matches(
            authorization, work_item_id=work_item_id, provider=self.name, status=status,
        ):
