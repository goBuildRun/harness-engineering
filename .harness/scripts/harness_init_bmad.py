...
BMAD_CONFIG_BEGIN = "# BEGIN harness-engineering managed BMAD output"
BMAD_CONFIG_END = "# END harness-engineering managed BMAD output"
...
def maybe_install_bmad(product: dict[str, Any], install: bool) -> str:
    ...
    modules = os.environ.get("HARNESS_BMAD_MODULES", "bmm,tea")
    tools = os.environ.get("HARNESS_BMAD_TOOLS", "codex,cursor")
    communication_language = os.environ.get("HARNESS_BMAD_COMMUNICATION_LANGUAGE", "Chinese")
    document_language = os.environ.get("HARNESS_BMAD_DOCUMENT_LANGUAGE", "Chinese")
    user_name = os.environ.get("HARNESS_BMAD_USER_NAME", str(product.get("name") or product.get("id") or "BMad"))
...
# BMAD installer only accepts the project root. harness-engineering pins
...
def upsert_managed_block(text: str, block: str) -> str:
    if BMAD_CONFIG_BEGIN in text and BMAD_CONFIG_END in text:
        before, rest = text.split(BMAD_CONFIG_BEGIN, 1)
        _old, after = rest.split(BMAD_CONFIG_END, 1)
