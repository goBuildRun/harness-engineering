...
PLAN_PATHS=$(printf '%s\n' "$PLAN_TEXT" | python3 "$SCRIPT_DIR/business_paths.py" extract --harness-root "$HARNESS_ROOT" --product-root "$PRODUCT_ROOT" 2>/dev/null || true)

# 须含业务路径或 structure_guard
if [[ -z "$PLAN_PATHS" && ! "$PLAN_TEXT" =~ structure_guard ]]; then
  ROOTS=$(python3 "$SCRIPT_DIR/business_paths.py" roots --harness-root "$HARNESS_ROOT" --product-root "$PRODUCT_ROOT" 2>/dev/null | tr '\n' ' ' || true)
