...
def load_allowlist(rules_path: Path) -> dict:
    if yaml is None or not rules_path.is_file():
        return {
            "allowed_roots": list(BUSINESS_PREFIXES) + ["harness-engineering/"],
...
def main() -> int:
    ...
    rules_path = allowlist_path(harness_root, args.profile, product_root)
    if not rules_path.is_file():
        emit("block", f"UNKNOWN_PROFILE: 找不到 profile allowlist: {rules_path}")
        return 0
    cfg = load_allowlist(rules_path)
    business_roots = load_business_roots(harness_root, args.profile, product_root)
    ...
    if args.diff and not args.path:
        ...
        harness_only = [p for p in paths if p.startswith("harness-engineering/")]
        if not business_paths:
            if harness_only:
                emit("pass", f"STRUCTURE_SKIP_HARNESS_ONLY: 仅 harness 变更 {len(harness_only)} 个文件")
            else:
    ...
    for rel in paths:
        rel = normalize(rel)
        if rel.startswith("harness-engineering/"):
            rel = rel[len("harness-engineering/") :]
