...
from harness_knowledge import sync_planning  # noqa: E402
...
class HarnessPlanningTest(unittest.TestCase):
    ...
    def test_plan_batch_writes_children_without_shared_active_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            child = product / "harness-workspace/runs/planning" / result["batch_id"] / "children/WI-demo.json"
            self.assertTrue(child.is_file())
            activated = activate_task(layout, "WI-demo", task_scoped=True)
            self.assertTrue(activated["ok"])
            self.assertTrue((product / "harness-workspace/runs/tasks/WI-demo/planning_gate_pass.json").is_file())
    ...
    def test_knowledge_sync_uses_input_digest_cache(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cache = product / "harness-workspace/runs/planning/context-sync.json"
    ...
    def test_global_preflight_short_circuits_incomplete_l3_batch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse((product / "harness-workspace/runs/planning").exists())

    def test_cli_plan_uses_single_batch_command(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            product, task = self._product(Path(tmp))
            completed = subprocess.run(
                ["bash", str(SCRIPTS / "harness"), "--product-root", str(product),
                 "plan", "--level", "L3", "--task-dir", str(task)],
                cwd=ROOT,
                env={**os.environ, "HARNESS_PRODUCT_ROOT": str(product), "HARNESS_PRETTY": "0"},
            ...
            started = subprocess.run(
                ...
                env={**os.environ, "HARNESS_PRODUCT_ROOT": str(product),
                     "WORK_ITEM_PROVIDER": "noop", "HARNESS_PRETTY": "0"},
            ...
            self.assertFalse((product / "harness-workspace/runs/active_task.json").exists(),
                             ...
                             if (product / "harness-workspace/runs/active_task.json").exists() else "")
