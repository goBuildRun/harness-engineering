...
from ael_gates import (  # noqa: E402
...
from ael_gate_work_items import validate_ci_planning_credential  # noqa: E402
from ael_provider_preflight import execute_preflight  # noqa: E402
from ael_execution_authority import AuthorityTrust, build_receipt, sign_receipt  # noqa: E402
...
from ael_strict_gate import COMPONENT_SCHEMA  # noqa: E402
...
class HarnessGatesTest(unittest.TestCase):
    ...
    def test_gate_plan_deadline_covers_input_digest_before_execution(self) -> None:
        ...
        runner = mock.Mock()
        with mock.patch("ael_gates.run_process_group", runner):
    ...
    def test_ci_result_resolves_work_item_separately_from_task_id(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            product = Path(tmp)
            task = product / "ael-workspace/planning/tasks/task-1"
    ...
    def test_same_id_merges_provider_but_conflicting_providers_are_ambiguous(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            task = product / "ael-workspace/planning/tasks" / task_id
            task.mkdir(parents=True)
            (task / "task.json").write_text(json.dumps({
    ...
    def test_standard_frontend_without_url_blocks_browser_gate(self) -> None:
        ...
        with mock.patch("ael_gates.run_process_group", return_value=completed), \
                mock.patch("ael_gates.active_planning_gate_path", return_value=credential), \
    ...
    def test_ci_preparation_failure_blocks_before_gate_execution(self) -> None:
        runner = mock.Mock()
        with mock.patch("ael_gates.prepare_ci_task", return_value=False), \
                mock.patch("ael_gates.run_process_group", runner):
            result = run_gate_plan(
                ROOT, ROOT, tier="standard", subject_digest="subject", policy_digest="policy",
    ...
    def test_stale_active_ci_credential_cannot_be_reused_for_another_task(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (runs / "planning_gate_pass.json").write_text(json.dumps({
                ...
                "task_dir": str(product / "ael-workspace/planning/tasks/old-task"),
    ...
    def test_ci_task_materializes_committed_planning_credential(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            materialized = json.loads((
                product / "ael-workspace/runs/ci/task-1/planning_gate_pass.json"
            ).read_text())
            ...
            self.assertEqual(
                ...
                "ael-workspace/planning/tasks/task-1/planning_gate_pass.json",
            )
            self.assertEqual(
    ...
    def test_ci_task_resolves_slug_directory_by_work_item_identity(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            product = Path(tmp)
            task = product / "ael-workspace/planning/tasks/2026-08-14-WI-42-story"
            task.mkdir(parents=True)
            (task / "planning_gate_pass.json").write_text(json.dumps({
                "decision": "pass", "work_item": {"id": "WI-42", "provider": "feishu"},
            }))
            ...
            materialized = json.loads((
                product / "ael-workspace/runs/ci/WI-42/planning_gate_pass.json"
            ).read_text())
            self.assertEqual(materialized["task_dir"], str(task.resolve()))
    ...
    def test_two_ci_tasks_prepare_concurrently_without_shared_state(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.dict(os.environ, {
                "AEL_CI_TASK_ID": "task-a",
                "AEL_CI_PLANNING_GATE": str(runs / "planning_gate_pass.json"),
            }, clear=True):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for expected_reason, payload in variants.items():
                with self.subTest(reason=expected_reason):
                    ...
                    runner = mock.Mock()
                    with mock.patch("ael_gates.prepare_ci_task", return_value=True), \
                            mock.patch("ael_gates.run_process_group", runner):
                        result = run_gate_plan(
                            ROOT, product, tier="standard", subject_digest="subject",
    ...
    def test_ci_gate_plan_propagates_task_specific_environment(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            completed = mock.Mock(returncode=0, stdout='{"decision":"pass","reason":"OK"}')
            with mock.patch("ael_gates.run_process_group", return_value=completed) as runner:
                result = run_gate_plan(
                    ROOT, product, tier="standard", subject_digest="subject",
                    policy_digest="policy", ci_task_id="task-1",
                )
            ...
            for call in runner.call_args_list:
                self.assertEqual(call.kwargs["env"]["AEL_CI_TASK_ID"], "task-1")
                self.assertEqual(call.kwargs["env"]["AEL_CI_PLANNING_GATE"], expected)
    ...
    def test_local_finish_task_rejects_stale_planning_credential(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            runner = mock.Mock()
            with mock.patch(
                "ael_gates.active_planning_gate_path", return_value=credential,
            ), mock.patch("ael_gates.run_process_group", runner):
                result = run_gate_plan(
                    ROOT, product, tier="standard", subject_digest="subject",
    ...
    def test_standard_qa_failure_blocks_shared_result(self) -> None:
        ...
        credential = mock.Mock()
        credential.read_text.return_value = '{"decision":"pass"}'
        with mock.patch("ael_gates.run_process_group", side_effect=completed), \
                mock.patch("ael_gates.active_planning_gate_path", return_value=credential), \
    ...
    def test_gate_timeout_fails_closed_with_gate_identity(self) -> None:
        ...
        with mock.patch("ael_gates.run_process_group", side_effect=started) as runner, \
                mock.patch("ael_gates.active_planning_gate_path", return_value=credential), \
                mock.patch.dict(os.environ, {
                    "CI": "true", "AEL_GATE_TIMEOUT_SECONDS": "7",
                }, clear=True):
            result = run_gate_plan(
                ROOT, ROOT, tier="standard", subject_digest="subject", policy_digest="policy",
            )
    ...
    def test_knowledge_timeout_is_not_hidden_by_autosync(self) -> None:
        ...
        credential = mock.Mock()
        credential.read_text.return_value = '{"decision":"pass"}'
        with mock.patch("ael_gates._run_gate_command", side_effect=gate_result), \
                mock.patch("ael_gates.active_planning_gate_path", return_value=credential), \
    ...
    def test_strict_requires_subject_bound_production_evidence(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch("ael_gates.run_process_group", return_value=completed), \
                    mock.patch("ael_gates.active_planning_gate_path", return_value=credential), \
                    mock.patch.dict(os.environ, {"AEL_STRICT_EVIDENCE": str(receipt)}):
    ...
    def test_strict_receipt_decode_failure_is_structured_block(self) -> None:
        ...
        with mock.patch("ael_gates.run_process_group", return_value=mock.Mock(
            returncode=0, stdout='{"decision":"pass","reason":"OK"}',
        )), mock.patch("ael_gates.active_planning_gate_path", return_value=credential), \
                mock.patch("ael_gates._strict_evidence", side_effect=decode_error):
            result = run_gate_plan(
    ...
    def test_synthetic_canary_cannot_satisfy_declared_real_provider_requirement(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch("ael_gates.run_process_group", return_value=completed), \
                ...
                with mock.patch(
                    "ael_strict_gate.trust_from_installation",
