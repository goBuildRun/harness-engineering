...
import ael_gates  # noqa: E402


class HarnessCliExitContractTest(unittest.TestCase):
    def test_standalone_gate_plan_uses_decision_as_exit_status(self) -> None:
        with mock.patch.object(sys, "argv", ["ael_gates.py"]), mock.patch.object(
            ael_gates, "run_gate_plan",
            return_value={"decision": "block", "checks": {}, "missing": ["harness"]},
        ), mock.patch.object(ael_gates, "dump_json"):
            self.assertEqual(ael_gates.main(), 1)
    ...
    def test_standalone_efficiency_clis_align_block_decision_and_exit_code(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            product = Path(tmp)
            progress = product / "ael-workspace/evidence/progress"
            ...
            commands = (
                [
                    "python3", str(SCRIPTS / "ael_provider_preflight.py"),
                ...
                [
                    "python3", str(SCRIPTS / "ael_lifecycle_preflight.py"),
