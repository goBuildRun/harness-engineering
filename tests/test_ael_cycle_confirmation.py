...
import ael_cycle_confirmation  # noqa: E402
import ael_commands  # noqa: E402
from ael_cycle_commands import begin_stage, finish_stage, start_story_cycle  # noqa: E402
from ael_runtime import atomic_write_result, canonical_digest, default_result, load_result, result_path  # noqa: E402
from ael_timing import ledger_path, parse_time, read_events, utc_now  # noqa: E402
...
class HarnessCycleConfirmationTest(unittest.TestCase):
    ...
    def test_confirm_promotes_implicit_takeover_without_resetting_story_clock(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            emitted = []
            with mock.patch.object(ael_cycle_confirmation, "dump_json", side_effect=emitted.append):
                ael_cycle_confirmation.cmd_confirm(self.args(product))
    ...
    def test_explicit_confirmation_clock_precedes_lifecycle_and_overrides_default(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            product = Path(tmp)
            with (
                mock.patch.object(ael_cycle_confirmation, "now", side_effect=confirmation_time),
                mock.patch.object(ael_cycle_confirmation, "discover_lifecycle", side_effect=lifecycle),
                mock.patch.object(ael_cycle_confirmation, "policy_for", return_value="policy"),
                mock.patch.object(ael_cycle_confirmation, "capture_baseline", side_effect=baseline),
                mock.patch.object(ael_cycle_confirmation, "capture_usage_baseline"),
                mock.patch.object(ael_cycle_confirmation, "automatic_receipt", return_value={}),
                mock.patch.object(ael_cycle_confirmation, "write_phase_handoff", return_value={}),
                mock.patch.object(ael_cycle_confirmation, "dump_json"),
                mock.patch("ael_timing.utc_now", return_value=implicit_start),
            ):
                ael_cycle_confirmation.cmd_confirm(self.args(
    ...
    def test_omitted_provider_uses_committed_binding_and_baseline_work_item(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                mock.patch.object(
                    ael_cycle_confirmation, "committed_work_item_resolution",
                    return_value={
                ...
                mock.patch.object(
                    ael_cycle_confirmation, "discover_lifecycle",
                ...
                ael_cycle_confirmation.cmd_confirm(args)
    ...
    def test_conflicting_active_task_blocks_second_confirmation(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(
                ael_cycle_confirmation, "dump_json", side_effect=emitted.append,
            ):
                ael_cycle_confirmation.cmd_confirm(self.args(product, "second-task"))
    ...
    def test_start_serializes_shared_active_task_before_task_result(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                mock.patch.object(
                    ael_commands, "committed_work_item_resolution",
                    return_value={"status": "missing"},
                ),
                mock.patch.object(
                    ael_commands, "resolve_start_work_item",
                    return_value={"decision": "pass", "work_item": None},
                ),
                mock.patch.object(ael_commands, "task_operation_lock", side_effect=Lock),
                mock.patch.object(ael_commands, "_cmd_start_locked", return_value=0),
            ):
                ael_commands.cmd_start(args)

        self.assertEqual(
            locked,
            [
                product / "ael-workspace/runs/active_task.json",
