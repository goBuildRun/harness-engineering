...
from ael_assurance import (audit_guards, check_bootstrap, check_release_candidate,
                               create_bootstrap, create_release_candidate, install_guards,
                               pre_push_script)  # noqa: E402
from ael_runtime import apply_code_health, default_result, load_result  # noqa: E402
from ael_commands import refresh_assurance  # noqa: E402
from ael_schema import validate_result  # noqa: E402
from ael_cli import main as harness_main  # noqa: E402
...
class HarnessAssuranceTest(unittest.TestCase):
    ...
    def test_local_lite_onboarding_public_path_completes_under_five_minutes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run(["git", "commit", "-qm", "base"], cwd=product, check=True)
            command = [str(SCRIPTS / "ael"), "--product-root", str(product)]
    ...
    def test_finish_assurance_refresh_reports_guarded_pre_commit_head(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("ael_cycle_commands.verify_attestation") as verify:
    ...
    def test_guard_install_is_repo_local_versioned_and_auditable(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name in ("pre-commit", "post-commit", "pre-push"):
                ...
                self.assertIn("AEL_GUARD_RUNTIME_MISSING", hook.read_text())
            subprocess.run(["git", "add", ".githooks"], cwd=product, check=True)
    ...
    def test_guard_blocks_without_validated_task_and_finds_configured_runtime(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotIn("AEL_GUARD_RUNTIME_MISSING", blocked.stderr)
    ...
    def test_guarded_finish_commit_push_lifecycle_does_not_stale_deadlock(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (product / "ael-workspace/runs/active_task.json").write_text(
            ...
            binding_result = json.loads(subprocess.check_output(
                [*command, "finish", "--skip-legacy-gates"], text=True,
                env={**dict(os.environ), "AEL_PRODUCT_ROOT": str(product)},
            ))
            self.assertEqual(binding_result["decision"], "pass")
            active = json.loads((product / "ael-workspace/runs/active_task.json").read_text())
            ...
            self.assertIn("AEL_PUSH_GUARD_PASS", pushed.stderr)
            status = json.loads(subprocess.check_output([*command, "status"], text=True))
    ...
    def test_hooks_fail_closed_when_active_task_has_no_identity(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("AEL_GUARD_TASK_ID_MISSING", post_commit.stderr)
            ...
            self.assertIn("AEL_GUARD_TASK_ID_MISSING", pre_push.stderr)
            ...
            self.assertIn("AEL_GUARD_TASK_ID_MISSING", missing_result.stderr)

            task_root = product / "ael-workspace/runs/tasks/missing-state"
            task_root.mkdir(parents=True)
            ...
            self.assertIn("AEL_GUARD_TASK_ID_MISSING", conflicting_identity.stderr)
            active.write_text(json.dumps({"work_item_id": "missing-state"}), encoding="utf-8")
            ...
            self.assertIn("AEL_GUARD_BASELINE_MISSING", missing_baseline.stderr)
            ...
            self.assertIn("AEL_GUARD_TASK_ID_MISSING", empty_range_missing_result.stderr)
            ...
            self.assertIn("AEL_PUSH_GUARD_BLOCKED", empty_range_unattested_tip.stderr)
    ...
    def test_guard_override_uses_one_effective_runtime_root(self) -> None:
        script = pre_push_script()
        self.assertIn('AEL_RUNTIME_ROOT="${AEL_ENGINEERING_ROOT:-$CONFIGURED_ROOT}"', script)
        self.assertIn('ATTEST="$AEL_RUNTIME_ROOT/.ael/scripts/ael_attestation.py"', script)
        self.assertNotIn('$CONFIGURED_ROOT/.ael/scripts/', script)
