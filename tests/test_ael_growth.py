...
from ael_growth import (  # noqa: E402
    apply_review,
    capture_evidence,
    collect,
    freshness_status,
...
class HarnessGrowthTest(unittest.TestCase):
    def test_growth_rejects_invalid_identity_before_creating_workspace(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            completed = subprocess.run(
                [
                    "bash", str(SCRIPT_DIR / "ael_growth.sh"), "capture",
    ...
    def test_cross_day_scan_preserves_unchanged_reviewed_candidates(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            completed = subprocess.run(
                [
                    "bash", str(SCRIPT_DIR / "ael_growth.sh"), "scan",
    ...
    def test_capture_evidence_writes_progress_candidate_scannable_by_growth(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            capture = capture_evidence(
                ...
                command="bash .ael/scripts/work_item.sh feishu-tasklist-member",
            )
...
- **来源**：`ael-workspace/evidence/review-reports/demo-T1-REVIEW.md`
...
- **来源**：`ael-workspace/evidence/test-reports/demo-T1-TEST.md`
- **原文摘要**：缺少失败路径测试会造成返工。
- **建议分类**：lesson / context / architecture / tech-debt / ignore
...
- **来源**：`ael-workspace/evidence/progress/demo-T1-PROGRESS.md`
- **原文摘要**：一次性调试日志，无长期价值。
- **建议分类**：lesson / context / architecture / tech-debt / ignore
- **人工决定**：ignore
- **处理结果**：已忽略
""".lstrip(),
        ...
        self.assertIn("BEGIN buildrun-agent-engineering-lifecycle:growth:context", context)
        self.assertIn("ProductClient", context)
        self.assertIn("BEGIN buildrun-agent-engineering-lifecycle:growth:lessons", lessons)
...
""".lstrip(),
        ...
        self.assertEqual(status["pending"], ["ael-workspace/evidence/growth-reports/2026-06-22-GROWTH.md:2"])
...
- **来源**：`ael-workspace/evidence/summaries/demo-T1-SUMMARY.md`
...
""".lstrip(),
    ...
    def test_apply_review_cli_ignore_does_not_create_missing_knowledge_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            product = Path(tmp)
            write_project_config(product)
            report = product / "ael-workspace/evidence/growth-reports/2026-06-22-GROWTH.md"
...
""".lstrip(),
            ...
            context_exists = (product / "ael-workspace/knowledge/CONTEXT.md").exists()
            lessons_exists = (product / "ael-workspace/knowledge/LESSONS.md").exists()
...
""".lstrip(),
        ...
        self.assertEqual(
            ...
            [
                "ael-workspace/knowledge/CONTEXT.md",
                "ael-workspace/knowledge/LESSONS.md",
            ],
        )
        self.assertNotIn("BEGIN buildrun-agent-engineering-lifecycle:growth", context)
        self.assertNotIn("BEGIN buildrun-agent-engineering-lifecycle:growth", lessons)
...
""".lstrip()
        ...
        self.assertEqual(after.count("BEGIN buildrun-agent-engineering-lifecycle:growth:context"), 1)
    ...
    def test_review_status_preserves_historical_pending_without_blocking_current_report(self) -> None:
        ...
        self.assertEqual(status["pending"], [])
        self.assertEqual(status["historical_pending"], ["ael-workspace/evidence/growth-reports/2026-06-21-GROWTH.md:2"])
        self.assertEqual(status["current_report"], "ael-workspace/evidence/growth-reports/2026-06-22-GROWTH.md")
