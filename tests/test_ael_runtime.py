...
from ael_runtime import (  # noqa: E402
    SCHEMA_VERSION,
    apply_code_health,
    atomic_write_result,
    cache_matches,
...
from ael_cycle_commands import begin_stage, finish_stage  # noqa: E402
from ael_scope import paths_within_scope  # noqa: E402
from ael_state import invalidate_if_stale  # noqa: E402
from ael_task_resolution import bind_active_task  # noqa: E402
import ael_commands  # noqa: E402
import ael_ci  # noqa: E402
import ael_cli  # noqa: E402
import ael_cycle_release  # noqa: E402
import ael_cycle_usage  # noqa: E402
...
class HarnessRuntimeTest(unittest.TestCase):
    def test_finish_facade_injects_live_commands_namespace(self) -> None:
        ...
        with mock.patch.object(
            ael_commands.ael_finish, "execute_finish", return_value=7,
        ) as execute_finish:
            self.assertEqual(ael_commands.cmd_finish(args), 7)
        execute_finish.assert_called_once_with(args, commands=ael_commands)
    ...
    def test_finish_locks_shared_active_task_before_task_result(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            args = SimpleNamespace(
                product_root=str(product), ael_root=str(ROOT), task_id="task-1",
            )
            with mock.patch.object(
                ael_commands.ael_finish, "_execute_finish_locked", return_value=0,
            ):
                ael_commands.ael_finish.execute_finish(args, commands=commands)
    ...
    def test_start_resume_monotonically_strengthens_tier_and_scope(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            captured = []
            with mock.patch.object(ael_commands, "dump_json", side_effect=captured.append):
                ael_commands.cmd_start(SimpleNamespace(
                    **common, tier="standard", scope=["ael-workspace/planning/tasks/story"],
                ))
            baseline_path = product / "ael-workspace/runs/tasks/resume-strict/worktree_baseline.json"
            baseline_before = baseline_path.read_bytes()

            captured = []
            with mock.patch.object(ael_commands, "dump_json", side_effect=captured.append):
                ael_commands.cmd_start(SimpleNamespace(
                    **common, tier="strict", scope=["deer-flow", "ael-workspace/evidence"],
                ))
            ...
            self.assertEqual(result["task"]["scope"], [
                ...
                "ael-workspace/planning/tasks/story",
                "deer-flow",
            ])
    ...
    def test_start_resume_rejects_work_item_rebinding(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(ael_commands, "dump_json"):
                ael_commands.cmd_start(SimpleNamespace(**common, work_item="WI-1"))
            captured = []
            with mock.patch.object(ael_commands, "dump_json", side_effect=captured.append):
                ael_commands.cmd_start(SimpleNamespace(**common, work_item="WI-2"))
    ...
    def test_start_resume_repairs_null_work_item_from_unique_planning(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            common = {
                "product_root": str(product), "ael_root": str(ROOT), "task_id": task_id,
                "work_item": "", "kind": "implementation", "reason": "",
                "tier": "strict", "scope": ["src"],
            }
            with mock.patch.object(ael_commands, "dump_json"):
                ael_commands.cmd_start(SimpleNamespace(**common))
            path = product / "ael-workspace/runs/tasks" / task_id / "result.json"
            ...
            task_dir = product / "ael-workspace/planning/tasks/2026-resume-planning-binding"
    ...
    def test_start_infers_unique_committed_work_item_and_preserves_provider(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run(["git", "init", "-q"], cwd=product, check=True)
            task_id = "WI-43.3"
            task_dir = product / "ael-workspace/planning/tasks/2026-08-19-story-43-3"
            task_dir.mkdir(parents=True)
            (product / "ael-workspace/project.yaml").write_text(
                "work_item:\n  provider: feishu\n  providers:\n    feishu:\n      status_update_mode: completed\n",
                encoding="utf-8",
    ...
    def test_start_rejects_explicit_work_item_conflicting_with_committed_planning(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            task_dir = product / "ael-workspace/planning/tasks" / task_id
            task_dir.mkdir(parents=True)
            ...
            self.assertFalse((
                product / "ael-workspace/runs/tasks" / task_id / "result.json"
            ).exists())
    ...
    def test_task_inference_blocks_ambiguity_without_active_pointer(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for task_id in ("task-a", "task-b"):
                target = product / "ael-workspace" / "runs" / "tasks" / task_id / "result.json"
    ...
    def test_runtime_task_ids_reject_path_traversal_without_writing(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(ael_commands, "dump_json", side_effect=captured.append):
                ael_commands.cmd_start(SimpleNamespace(
                    product_root=str(product), ael_root=str(ROOT), task_id="../../escape",
    ...
    def test_ci_task_environment_is_authoritative_and_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            product = Path(tmp)
            with mock.patch.dict(os.environ, {"AEL_CI_TASK_ID": "ci-task-1"}, clear=True):
                self.assertEqual(resolve_task_id(product, None), ("ci-task-1", []))
                self.assertEqual(
                    resolve_task_id(product, "other-task"),
                    ("", ["CI_TASK_ID_MISMATCH"]),
                )
            with mock.patch.dict(os.environ, {"AEL_CI_TASK_ID": "../escape"}, clear=True):
                self.assertEqual(resolve_task_id(product, None), ("", ["TASK_ID_INVALID"]))
    ...
    def test_amend_migrated_task_adds_audited_scope_and_invalidates_checks(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            captured = []
            with (mock.patch.object(ael_commands, "dump_json", side_effect=captured.append),
                  mock.patch.object(ael_commands, "policy_for", return_value="new-policy")):
                ael_commands.cmd_amend(SimpleNamespace(
                    product_root=str(product), ael_root=str(ROOT), task_id=task_id,
                    scope=["ael-workspace/", ".githooks", ".githooks"],
                    reason="recover committed adoption scope",
                ))
            amended = captured[-1]["result"]
            self.assertEqual(captured[-1]["reason"], "TASK_BINDING_AMENDED")
            self.assertEqual(amended["task"]["scope"], [".githooks", "ael-workspace"])
    ...
    def test_concurrent_amendments_preserve_both_binding_revisions(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            command = [
                sys.executable, str(SCRIPTS / "ael_runtime.py"),
    ...
    def test_usage_baseline_holds_task_operation_lock_for_update(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(
                ael_cycle_usage, "task_operation_lock", return_value=operation_lock,
            ) as lock_factory, mock.patch.object(
                ael_cycle_usage, "automatic_receipt", return_value=receipt,
            ), mock.patch.object(
                ael_cycle_usage, "changed_since_baseline", return_value=[],
            ), mock.patch.object(ael_cycle_usage, "dump_json"):
                ael_cycle_usage.cmd_usage_baseline(SimpleNamespace(
                    product_root=str(product), ael_root=str(ROOT), task_id=task_id,
    ...
    def test_cli_lite_start_status_finish_flow(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            command = [
                "python3", str(SCRIPTS / "ael_runtime.py"),
                "--ael-root", str(ROOT), "--product-root", str(product),
            ]
    ...
    def test_explicit_story_closes_only_after_candidate_commit_and_lifecycle_readback(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch.object(
                ...
                ael_cycle_release, "dump_json", side_effect=release_output.append,
            ):
    ...
    def test_status_projects_input_change_without_mutating_result(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            result_path = product / "ael-workspace/runs/tasks/read-only-status/result.json"
    ...
    def test_finish_fails_closed_when_a_gate_changes_the_subject(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            subprocess.run(["git", "add", "."], cwd=product, check=True)
            subprocess.run(["git", "commit", "-qm", "base"], cwd=product, check=True)
            common = {"product_root": str(product), "ael_root": str(ROOT)}
            with mock.patch.object(ael_commands, "dump_json"):
                ael_commands.cmd_start(SimpleNamespace(
            ...
            def mutate_subject(*_args, **kwargs):
                ...
                checks = {
                    name: ael_commands.executed_check(
                        decision="pass", fingerprint=f"{name}-fingerprint",
                        subject_digest=kwargs["subject_digest"],
                        policy_digest=kwargs["policy_digest"],
                        completed_at=ael_commands.now(),
                    )
                    for name in ("harness", "structure", "quality_lint", "quality_test")
                }
                return {"decision": "pass", "checks": checks, "missing": []}

            with mock.patch.object(ael_commands, "run_gate_plan", side_effect=mutate_subject), \
                    mock.patch.object(ael_commands, "dump_json", side_effect=captured.append):
                ael_commands.cmd_finish(SimpleNamespace(
                    **common, task_id="gate-mutation", skip_legacy_gates=False,
                ))

            outcome = captured[-1]
            result = outcome["result"]
            changed = ael_commands.changed_since_baseline(
                product,
                ael_commands.result_path(product, "gate-mutation").parent / "worktree_baseline.json",
            )
            self.assertEqual(outcome["decision"], "block")
            self.assertEqual(outcome["reason"], "INPUT_CHANGED")
            self.assertEqual(result["state"], "blocked")
            self.assertEqual(result["subject"]["digest"], ael_commands.subject_for(product, changed))
    ...
    def test_finish_crossing_total_deadline_persists_block_and_does_not_close_root(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            captured = []
            with mock.patch.object(
                ael_commands, "budget_status", side_effect=[ok, ok, expired],
            ), mock.patch.object(
                ael_commands, "dump_json", side_effect=captured.append,
            ):
                ael_commands.cmd_finish(SimpleNamespace(
    ...
    def test_ci_checks_are_always_executed(self) -> None:
        completed = subprocess.run([
            "python3", str(SCRIPTS / "ael_runtime.py"),
            "--ael-root", str(ROOT), "--product-root", str(ROOT),
        ...
        self.assertFalse((ROOT / "ael-workspace").exists())
    ...
    def test_tier_only_escalates_and_unknown_defaults_standard(self) -> None:
        ...
        self.assertEqual(task_kind_tier("ael-maintenance", "lite"), "lite")
        self.assertEqual(classify_tier([".githooks/pre-commit", "ael-workspace/project.yaml"],
                                       floor="lite", kind="ael-maintenance"), "lite")
        self.assertEqual(classify_tier(["src/a.py"], floor="lite", kind="ael-maintenance"), "standard")
    ...
    def test_lite_revalidation_discards_stale_standard_only_checks(self) -> None:
        ...
        retained = ael_commands.checks_for_tier(checks, "lite")
    ...
    def test_gc_change_reexecutes_tier_and_scope_for_new_subject(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def run_gc(*_args, **_kwargs):
                source.write_text("def live():\n    return 2\n")
                changed = ael_commands.changed_since_baseline(
                    product, ael_commands.result_path(product, "gc-cache").parent / "worktree_baseline.json"
                )
                return {
                    "decision": "pass",
                    "role": "gc-sweeper", "independent": True, "task_id": "gc-cache",
                    "subject_digest": ael_commands.subject_for(product, changed),
                    "policy_digest": ael_commands.policy_for(ROOT, product),
                    "findings": 1, "remediated": 1, "deferred_work_items": [],
                }

            with mock.patch.object(ael_commands, "invoke_gc_once", side_effect=run_gc), \
                    mock.patch.object(ael_commands, "dump_json", side_effect=captured.append):
                ael_commands.cmd_finish(SimpleNamespace(
    ...
    def test_ci_check_consumes_subject_bound_gc_receipt_and_telemetry(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            captured = []
            with mock.patch.object(ael_ci, "run_gate_plan", return_value={
                "decision": "pass", "checks": {}, "missing": [],
            }), mock.patch.object(ael_ci, "dump_json", side_effect=captured.append):
                ael_commands.cmd_ci_check(SimpleNamespace(
                    product_root=str(product), ael_root=str(ROOT), task_id="gc-ci",
    ...
    def test_validate_ael_has_no_product_workspace_side_effect(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            product = Path(tmp)
            env = {"AEL_PRODUCT_ROOT": str(product)}
            subprocess.run(
                ["bash", str(SCRIPTS / "validate_ael.sh")],
                cwd=ROOT,
                env={**__import__("os").environ, **env},
                check=True,
                stdout=subprocess.DEVNULL,
            )
            self.assertFalse((product / "ael-workspace").exists())
